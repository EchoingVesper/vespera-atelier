# PowerShell script to copy MCP configurations from global to project-specific settings
...
function Merge-McpConfigs {
    ...
    if (-not $serversToMove -or $serversToMove.Count -eq 0) {
        Write-Host "Available MCP servers in global config:"
        $serverIndex = 1
        $serverOptions = @{}
...
try {
    ...
    Merge-McpConfigs -sourceConfigPath $globalConfigPath -targetConfigPath $projectConfigPath
