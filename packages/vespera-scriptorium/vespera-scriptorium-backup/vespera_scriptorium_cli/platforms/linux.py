"""
Linux-specific implementation for Vespera Scriptorium CLI.

This module provides Linux-specific functionality for detecting and configuring
MCP clients on Linux operating systems.
"""
...
# Client configuration paths for Linux
CLIENT_PATHS = {
    "claude_desktop": {
...
def detect_clients():
    """
    Detect installed MCP clients on Linux.
...
def is_client_configured(client_id, config_path):
    """
    Check if a client is already configured for the Vespera Scriptorium.
...
def configure_client(client_id, server_path, server_name="Task Orchestrator"):
    """
    Configure a client to use the Vespera Scriptorium.
    
    Args:
        client_id (str): Client identifier
        server_path (str): Path to the Vespera Scriptorium server script
