"""
File writer utilities with staging support for Vespera Scriptorium.
...
class StreamingFileWriter:
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            is_valid, error_msg = StagingValidator.validate_file_path(
            ...
            self.request_id = await self.staging_manager.create_staging_operation(
                target_path=self.target_path, mode=WriteMode.STREAM
            )

            self._initialized = True
            logger.info(
                f"Initialized streaming writer for {self.target_path} (request: {self.request_id})"
            )
            return True
    ...
    async def _backup_original(self) -> None:
        ...
        try:
            await StagingUtils.safe_file_operation(self.target_path.rename, backup_path)
    ...
    async def add_file(self, file_path: Union[str, Path], content: str) -> bool:
        ...
        try:
            ...
            request_id = await self.staging_manager.create_staging_operation(
                target_path=file_path, mode=WriteMode.BATCH
            )
...
async def write_file_atomically(
    file_path: Union[str, Path], content: str, config: Optional[WriteConfig] = None
) -> bool:
    """
...
async def write_files_batch(
    files: Dict[Union[str, Path], str], config: Optional[WriteConfig] = None
) -> bool:
    """
