"""
Utilities for staging operations in Vespera Scriptorium.
...
class StagingUtils:
    ...
    async def validate_file_integrity(
        ...
        """
        try:
            actual_checksum = await StagingUtils.calculate_checksum(
                file_path, algorithm
            )
        ...
        try:
            ...
            if system == "Windows":
                ...
                result = subprocess.run(
                    ["df", "-T", str(path)], capture_output=True, text=True
                )
    ...
    async def safe_file_operation(
        operation_func, *args, max_retries: int = 3, retry_delay: float = 0.5, **kwargs
    ) -> Any:
        """
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                last_exception = e
                logger.warning(
                    f"File operation failed (attempt {attempt + 1}/{max_retries + 1}): {str(e)}"
                )
...
class StagingValidator:
    ...
    def validate_content_size(content: str, max_size_mb: int = 100) -> Tuple[bool, str]:
        ...
        if content_size > max_size_bytes:
            return (
                False,
                f"Content size ({StagingUtils.format_file_size(content_size)}) exceeds maximum allowed size ({max_size_mb} MB)",
            )
...
async def create_safe_staging_directory(
    base_dir: Path, request_id: str
) -> Optional[Path]:
    """
    ...
    staging_path = (
        base_dir / f"staging_{request_id}_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}"
    )
