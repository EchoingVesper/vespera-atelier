"""
Core staging manager for temporary file operations in Vespera Scriptorium.
...
class StagingManager:
    ...
    def __init__(
        self,
        base_staging_dir: Optional[Path] = None,
        cleanup_interval: int = 3600,
        max_staging_age_hours: int = 24,
    ):
        """
        ...
        """
        self.base_staging_dir = base_staging_dir or Path(".vespera_scriptorium/staging")
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            if not await StagingUtils.ensure_directory(self.base_staging_dir):
                logger.error(
                    f"Failed to create staging directory: {self.base_staging_dir}"
                )
                return False
            ...
            logger.info(
                f"Staging manager initialized with directory: {self.base_staging_dir}"
            )
            return True
    ...
    async def create_staging_operation(
        self,
        ...
        staging_dir = await create_safe_staging_directory(
            self.base_staging_dir, request_id
        )
        if staging_dir is None:
            raise RuntimeError(
                f"Failed to create staging directory for request: {request_id}"
            )
    ...
    async def write_content(
        self, request_id: str, content: str, append: bool = False
    ) -> bool:
        """
        ...
        async with self.operation_locks[request_id]:
            ...
            try:
                ...
                estimated_time = StagingUtils.estimate_operation_time(
                    content_bytes, "write"
                )
                ...
                async with aiofiles.open(
                    operation.staging_path, mode, encoding="utf-8"
                ) as f:
                ...
                if operation.mode != WriteMode.STREAM:
                    operation.checksum = await StagingUtils.calculate_checksum(
                        operation.staging_path
                    )

                logger.debug(
                    f"Wrote {len(content)} characters to staging file for {request_id}"
                )
                return True
    ...
    async def stream_writer(self, request_id: str):
        ...
        async with self.operation_locks[request_id]:
            ...
            try:
                ...
                async with aiofiles.open(
                    operation.staging_path, "w", encoding="utf-8"
                ) as f:
                    yield f
    ...
    async def commit_operation(
        self, request_id: str, validate_integrity: bool = True
    ) -> bool:
        """
        ...
        async with self.operation_locks[request_id]:
            ...
            try:
                ...
                if validate_integrity and operation.checksum:
                    ...
                    if not is_valid:
                        await self._mark_operation_failed(
                            operation, "File integrity validation failed"
                        )
                        return False
                ...
                if not await StagingUtils.ensure_directory(target_dir):
                    await self._mark_operation_failed(
                        operation, f"Failed to create target directory: {target_dir}"
                    )
                    return False
                ...
                logger.info(
                    f"Successfully committed operation {request_id} to {operation.target_path}"
                )
                return True
    ...
    async def list_operations(
        self, status_filter: Optional[StagingStatus] = None
    ) -> List[Dict[str, Any]]:
        """
    ...
    async def get_staging_statistics(self) -> Dict[str, Any]:
        ...
        if self.operations:
            ...
            operation_stats["average_operation_age_minutes"] = total_age_minutes / len(
                self.operations
            )
                ...
                if staging_dir.is_dir() and staging_dir.name.startswith("staging_"):
                    try:
                        ...
                        if len(parts) >= 2:
                            ...
                            if await self._recover_single_operation(
                                staging_dir, request_id
                            ):
                                recovered_count += 1
                    except Exception as e:
                        logger.warning(
                            f"Failed to recover operation from {staging_dir}: {str(e)}"
                        )
    ...
    async def _recover_single_operation(
        self, staging_dir: Path, request_id: str
    ) -> bool:
            ...
            for staging_file in staging_files:
                if staging_file.is_file():
                    operation = StagingOperation(
                        ...
                        target_path=Path(
                            "unknown"
                        ),  # Would need to be saved in metadata
    ...
    async def _mark_operation_failed(
        self, operation: StagingOperation, error_message: str
    ) -> None:
    ...
    async def _cleanup_operation(self, operation: StagingOperation) -> None:
        ...
        try:
            ...
            logger.warning(
                f"Error cleaning up operation {operation.request_id}: {str(e)}"
            )
