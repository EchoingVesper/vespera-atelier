...
from .base import LifecycleConfig, TaskLifecycleState
            ...
            with self.state_manager.session_scope() as session:
                ...
                archive_data = {
                    ...
                    "archive_metadata": {
                        ...
                        "stale_reasons": (
                            task_info.get("stale_reasons", []) if task_info else []
                        ),
                        "recommended_action": (
                            task_info.get("recommended_action", "unknown")
                            if task_info
                            else "unknown"
                        ),
                    },
...
"""
            ...
            self.logger.warning(
                f"Failed to create archive artifact for task {task.task_id}: {e}"
            )
            ...
            with self.state_manager.session_scope() as session:
                ...
                for archive in expired_archives:
                    try:
                        ...
                        self.logger.warning(
                            f"Error cleaning archive {archive.archive_id}: {e}"
                        )
