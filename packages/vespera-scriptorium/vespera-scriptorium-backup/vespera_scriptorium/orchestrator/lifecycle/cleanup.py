...
from ...db.models import MaintenanceOperationModel, SubTaskModel, TaskBreakdownModel
...
class WorkspaceCleanupManager:
    ...
    async def perform_workspace_cleanup(
        self, cleanup_type: str = "standard", aggressive: bool = False
    ) -> Dict[str, Any]:
    ...
    async def _cleanup_temporary_files(self) -> Dict[str, Any]:
        ...
        try:
            ...
            temp_dirs = [
                Path(".vespera_scriptorium/temp"),
                Path(".vespera_scriptorium/staging"),
            ...
            for temp_dir in temp_dirs:
                if temp_dir.exists():
                    freed_space = await self._clean_directory(
                        temp_dir, max_age_hours=24
                    )
    ...
    async def _cleanup_orphaned_artifacts(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for artifact_id in orphaned_artifacts:
                try:
                    delete_result = await self.artifact_manager.delete_artifact(
                        artifact_id
                    )
    ...
    async def _cleanup_incomplete_workflows(
                ...
                if workflow_info.get("should_cleanup", False) or aggressive:
                    try:
                        ...
                        if resolution_result.get("success"):
                            ...
                            result["items_cleaned"] += resolution_result.get(
                                "tasks_processed", 0
                            )
                        else:
                            result["errors"].append(
        ...
        try:
            with self.state_manager.session_scope() as session:
                ...
                if completion_rate > 0.7 and aggressive:
                    ...
                    result = await self._archive_incomplete_workflow(
                        subtasks, cleanup_reasons
                    )
            ...
            return {"success": False, "error": str(e), "parent_task_id": parent_task_id}

    async def _complete_remaining_workflow_tasks(
        self, subtasks: List[SubTaskModel], session
    ) -> Dict[str, Any]:
        ...
        for task in subtasks:
            if task.status in ["pending", "active"]:
                try:
                    ...
                    task.summary = (
                        "Task completed automatically to resolve incomplete workflow"
                    )
                    ...
                    self.logger.warning(
                        f"Failed to auto-complete task {task.task_id}: {e}"
                    )

        session.commit()

        return {"success": True, "completed_tasks": completed_count}

    async def _archive_incomplete_workflow(
        self, subtasks: List[SubTaskModel], cleanup_reasons: List[str]
    ) -> Dict[str, Any]:
