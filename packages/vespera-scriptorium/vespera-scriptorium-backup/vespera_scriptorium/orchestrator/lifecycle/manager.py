...
from .base import LifecycleConfig, StaleTaskReason, TaskLifecycleState
...
class TaskLifecycleManager:
    ...
    def __init__(
        ...
        self.archival_manager = TaskArchivalManager(
            state_manager, artifact_manager, self.config
        )
        self.cleanup_manager = WorkspaceCleanupManager(
            state_manager, artifact_manager, self.config
        )
    ...
    async def archive_task(
        self,
        task_id: str,
        archive_reason: str = "stale_timeout",
    ...
    async def bulk_archive_tasks(
        self,
        task_ids: List[str],
        archive_reason: str = "bulk_cleanup",
    ...
    async def perform_workspace_cleanup(
        ...
        """
        return await self.cleanup_manager.perform_workspace_cleanup(
    ...
    async def comprehensive_maintenance(
        ...
        try:
            ...
            stale_operation = {
                ...
                "critical_count": len(
                    [t for t in stale_tasks if t.get("max_severity") == "critical"]
                ),
                "high_count": len(
                    [t for t in stale_tasks if t.get("max_severity") == "high"]
                ),
            }
            ...
            if stale_tasks:
                ...
                if eligible_task_ids:
                    archive_results = await self.bulk_archive_tasks(
                        eligible_task_ids,
                        "maintenance_cleanup",
                        preserve_artifacts=not aggressive,
                    )
                    results["operations"].append(
                        {"operation": "stale_task_archival", **archive_results}
                    )

            # Step 3: Workspace cleanup
            self.logger.info("Performing workspace cleanup...")
            cleanup_results = await self.perform_workspace_cleanup(
            ...
            results["operations"].append(
                {"operation": "expired_archive_cleanup", **archive_cleanup}
            )
    ...
    async def get_lifecycle_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            medium_priority_tasks = [
                t for t in stale_tasks if t.get("max_severity") == "medium"
            ]
                ...
                "stale_task_summary": {
                    ...
                    "auto_cleanup_eligible": len(
                        [
                            ...
                            if t.get("auto_cleanup_eligible", False)
                        ]
                    ),
                },
                "archive_summary": {
    ...
    async def schedule_maintenance(
        self, maintenance_type: str = "standard", delay_hours: int = 0
    ) -> Dict[str, Any]:
        ...
        maintenance_id = (
            f"scheduled_{maintenance_type}_{scheduled_time.strftime('%Y%m%d_%H%M%S')}"
        )
