...
class RefactoredTaskOrchestrator:
    ...
    def __init__(
        ...
        config = {
            "database": {
                "url": "sqlite:///.vespera_scriptorium/orchestrator.db",
                "timeout": 30.0,
            }
        }
    ...
    async def plan_task(
        ...
        return await self.coordinator.plan_task(
            description, complexity, subtasks_json, context
        )
    ...
    async def complete_subtask_with_artifacts(
        ...
        # Complete the task
        return await self.coordinator.complete_subtask(task_id, results, artifacts)

    async def complete_subtask(
        self, task_id: str, results: str, specialist_type: Optional[str] = None
    ) -> Dict:
        """Complete a subtask and update its status."""
        return await self.coordinator.complete_subtask(task_id, results)
    ...
    async def get_status(self, include_completed: bool = False) -> Dict:
        ...
        return await self.coordinator.get_status(session_id, include_completed)
    ...
    async def _check_parent_task_progress(self, completed_task_id: str) -> Dict:
        ...
        completion = await self.coordinator.tracking_service.complete_task(
            completed_task_id, "", []  # Empty results as this is just checking
        )
...
def create_refactored_orchestrator(
    state_manager: StateManager,
    specialist_manager: SpecialistManager,
    project_dir: str = None,
) -> RefactoredTaskOrchestrator:
    """
    ...
    """
    return RefactoredTaskOrchestrator(state_manager, specialist_manager, project_dir)
