"""
Artifact management system for the Vespera Scriptorium.
...
class ArtifactManager:
    ...
    def store_artifact(
        ...
        return {
            ...
            "accessible_via": f".vespera_scriptorium/artifacts/{task_id}/{artifact_id}.md",
        }

    def _create_mirrored_structure(
        self, task_dir: Path, file_paths: List[str], artifact_id: str
    ) -> List[Path]:
    ...
    def _create_artifact_content(
        ...
        if file_paths:
            content_lines.extend(["## Referenced Files", ""])
            for file_path in file_paths:
        ...
        content_lines.extend(
            [
                ...
                "---",
                "",
                f"*This artifact was generated by the Vespera Scriptorium on {datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S UTC')}*",
                f"*Task ID: {task_id}*",
            ]
        )
    ...
    def _update_task_metadata_index(
                ...
                for artifact in artifacts:
                    ...
                    if created_at < cutoff_date:
                        ...
                        try:
                            ...
                            metadata_file = (
                                task_dir / f"{artifact['artifact_id']}_metadata.json"
                            )
                            if metadata_file.exists():
                            ...
                            cleaned_count += 1
                            logger.info(
                                f"Cleaned up old artifact {artifact['artifact_id']}"
                            )
                        except Exception as e:
                            logger.error(
                                f"Error cleaning artifact {artifact['artifact_id']}: {str(e)}"
                            )
