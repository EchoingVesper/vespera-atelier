"""
Streaming-aware artifact management system for the Vespera Scriptorium.
...
class StreamingArtifactManager:
    ...
    def __init__(self, base_dir: Optional[str] = None):
        ...
        self.persistence_dir = self.base_dir / ".vespera_scriptorium"
        ...
        logger.info(
            f"Initialized streaming artifact manager with base directory: {base_dir}"
        )
    ...
    async def resume_partial_session(
                    ...
                    if progress_file.exists():
                        try:
                            async with aiofiles.open(
                                progress_file, "r", encoding="utf-8"
                            ) as f:
                                progress_data = json.loads(await f.read())
                            ...
                            partial_sessions.append(
                                {
                                    ...
                                    "expected_size": progress_data.get(
                                        "expected_size_hint"
                                    ),
                                    "file_paths": progress_data["file_paths"],
                                }
                            ...
                            logger.error(
                                f"Error reading progress file {progress_file}: {str(e)}"
                            )
                            continue
        except Exception as e:
                    ...
                    if progress_file.exists():
                        try:
                            ...
                            if file_time < cutoff_time:
                                ...
                                cleaned_count += 1
                                logger.info(
                                    f"Cleaned up old temp directory: {artifact_dir}"
                                )
                        except Exception as e:
                            logger.error(
                                f"Error cleaning temp directory {artifact_dir}: {str(e)}"
                            )
        except Exception as e:
...
class StreamingSession:
    ...
    async def _initialize(self, resume: bool = False):
        ...
        if resume and self.resume_data:
            ...
            self.bytes_written = (
                self.temp_file.stat().st_size if self.temp_file.exists() else 0
            )
    ...
    async def write_stream(self):
        ...
        try:
            ...
            self.file_handle = await aiofiles.open(
                self.temp_file, mode, encoding="utf-8"
            )
    ...
    async def _create_artifact_header(self) -> str:
        ...
        if self.file_paths:
            header_lines.extend(["## Referenced Files", ""])
            for file_path in self.file_paths:
                header_lines.append(f"- `{file_path}`")
            header_lines.append("")

        header_lines.extend(
            ["## Detailed Work", "", "<!-- STREAMING CONTENT BEGINS HERE -->", ""]
        )
    ...
    async def _finalize(self):
        ...
        footer = [
            ...
            "---",
            "",
            f"*This artifact was generated by the Vespera Scriptorium on {self.last_update.strftime('%Y-%m-%d %H:%M:%S UTC')}*",
    ...
    async def _move_to_permanent_location(self):
        ...
        await self._create_metadata(
            final_path, mirrored_paths if self.file_paths else [final_path]
        )
        ...
        logger.info(
            f"Finalized streaming artifact {self.artifact_id} for task {self.task_id}"
        )
    ...
    def get_progress_info(self) -> Dict[str, Any]:
        ...
        if self.expected_size_hint and self.expected_size_hint > 0:
            progress_percent = min(
                100.0, (self.bytes_written / self.expected_size_hint) * 100
            )
...
async def create_streaming_artifact(
    base_dir: str,
    task_id: str,
    summary: str,
