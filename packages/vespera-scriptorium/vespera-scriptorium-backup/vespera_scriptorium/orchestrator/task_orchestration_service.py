...
class TaskOrchestrator:
    ...
    async def initialize_session(self) -> Dict:
        ...
        # Fall back to default Vespera Scriptorium definition
        return {
            "role": "Task Orchestrator",
            "capabilities": [
    ...
    @handle_errors(
        ...
        retry_policy=ExponentialBackoffPolicy(
            max_attempts=2, base_delay=0.1, backoff_factor=1.5
        ),
    ...
    async def plan_task(
        self, description: str, complexity: str, subtasks_json: str, context: str = ""
    ) -> Task:
        ...
        try:
            ...
            for i, st_data in enumerate(subtasks_data):
                ...
                task_id = st_data.get(
                    "task_id", f"{st_data['specialist_type']}_{uuid.uuid4().hex[:6]}"
                )
                subtask = Task(
                    task_id=task_id,
    ...
    async def _complete_subtask_with_artifacts_core(
        self,
        task_id: str,
        summary: str,
        artifacts: List[str],
    ...
    async def complete_subtask_with_artifacts(
        self,
        task_id: str,
        summary: str,
        artifacts: List[str],
        ...
        try:
            return await self._complete_subtask_with_artifacts_core(
                task_id, summary, artifacts, artifact_info
            )
    ...
    async def _complete_subtask_core(
        self, task_id: str, results: str, artifacts: List[str]
    ) -> Dict:
    ...
    async def complete_subtask(
        self, task_id: str, results: str, artifacts: List[str], next_action: str
    ) -> Dict:
    ...
    async def synthesize_results(self, parent_task_id: str) -> Dict[str, Any]:
        """Synthesize results with error recovery."""
        try:
            # Validate task ID format using domain service
            task_service = TaskService(None, None)  # Temporary instance for validation
            if not task_service.validate_task_id(parent_task_id):
                return {
            ...
            try:
                subtasks = await asyncio.wait_for(
                    self.state.get_subtasks_for_parent(parent_task_id), timeout=5
                )
            except asyncio.TimeoutError:
                logger.error(f"Timeout getting subtasks for task {parent_task_id}")
                return {
                    "status": "synthesis_failed",
                    "error": f"Timeout retrieving subtasks for task {parent_task_id}",
                    "parent_task_id": parent_task_id,
                    "recovery_suggestions": [
            ...
            try:
                ...
                return {
                    ...
                    "error": "Synthesis operation timed out",
                    "parent_task_id": parent_task_id,
                    "recovery_suggestions": [
    ...
    async def _get_status_core(self, include_completed: bool = False) -> Dict:
        ...
        all_tasks = await asyncio.wait_for(self.state.get_all_tasks(), timeout=5)
    ...
    async def _check_parent_task_progress(self, completed_task_id: str) -> Dict:
        ...
        try:
            ...
            logger.error(
                f"Timeout checking parent task progress for {completed_task_id}"
            )
    ...
    async def _get_next_recommended_task(
        ...
        try:
            ...
            for task in dependent_tasks:
                ...
                for dep_id in task.dependencies:
                    dep_task = next(
                        (st for st in subtasks if st.task_id == dep_id), None
                    )
            ...
            logger.error(
                f"Timeout getting next recommended task for {completed_task_id}"
            )
            return None
        except Exception as e:
