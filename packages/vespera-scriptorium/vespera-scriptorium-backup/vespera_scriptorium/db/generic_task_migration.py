...
class GenericTaskMigration:
    ...
    def migrate_task_breakdown(
        self, conn: sqlite3.Connection, migration_id: int, task_breakdown: sqlite3.Row
    ) -> str:
        ...
        """,
            (
                task_id,
                json.dumps({"from": "task_breakdown", "migration_id": migration_id}),
            ),
        )
    ...
    def migrate_subtask(
        ...
        hierarchy_level = (
            len(hierarchy_path.split("/")) - 2
        )  # Subtract empty string and root
    ...
    def _migrate_artifact(
        self, conn: sqlite3.Connection, task_id: str, artifact: Any, index: int
    ) -> None:
    ...
    def migrate_dependencies(self, conn: sqlite3.Connection) -> None:
        ...
        cursor.execute(
            "SELECT task_id, dependencies FROM subtasks WHERE dependencies != '[]'"
        )
        ...
        try:
            with self.get_connection() as conn:
                ...
                for task_breakdown in task_breakdowns:
                    try:
                        parent_id = self.migrate_task_breakdown(
                            conn, migration_id, task_breakdown
                        )
                        ...
                        for position, subtask in enumerate(subtasks):
                            self.migrate_subtask(
                                conn, migration_id, subtask, parent_path, position
                            )

                    except Exception as e:
                        logger.error(
                            f"Error migrating task breakdown {task_breakdown['parent_task_id']}: {e}"
                        )
                ...
                """,
                    (
                        datetime.now(),
                        self.migration_stats["task_breakdowns_migrated"]
                        + self.migration_stats["subtasks_migrated"],
