...
for the Vespera Scriptorium database using SQLAlchemy introspection.
"""
...
from .models import Base, SubTaskModel, TaskBreakdownModel
...
class MigrationManager:
    ...
    def __init__(self, database_url: str, models_module=None):
        ...
        self.metadata = (
            Base.metadata if models_module is None else models_module.metadata
        )
    ...
    def detect_schema_differences(self) -> SchemaDifference:
        ...
        try:
            ...
            logger.info(
                f"Schema analysis complete: {len(missing_tables)} missing tables, "
    ...
    def _compare_table_schema(self, table_name: str) -> List[str]:
        ...
        try:
            ...
            for col_name in set(model_columns.keys()).intersection(
                set(db_columns.keys())
            ):
    ...
    def generate_migration_operations(
        self, differences: SchemaDifference
    ...
    def execute_migrations(self, operations: List[MigrationOperation]) -> bool:
        ...
        logger.info(
            f"Starting migration {migration_id} with {len(operations)} operations"
        )
            ...
            with self.engine.connect() as conn:
                with conn.begin():
                    ...
                    for i, operation in enumerate(operations):
                        logger.info(
                            f"Executing operation {i+1}/{len(operations)}: "
    ...
    def _execute_single_operation(self, conn, operation: MigrationOperation) -> bool:
        ...
        try:
            if operation.operation_type == "CREATE_TABLE":
                ...
                sql = (
                    f"ALTER TABLE {operation.table_name} "
    ...
    def _record_migration_success(
        self, conn, migration_id: str, operations: List[MigrationOperation]
    ):
        ...
        operation_summary = str(
            [op.operation_type + op.table_name for op in operations]
        )
        checksum = hashlib.md5(operation_summary.encode()).hexdigest()

        # Combine rollback SQL
        rollback_sql = "; ".join(
            [op.rollback_sql for op in operations if op.rollback_sql]
        )
