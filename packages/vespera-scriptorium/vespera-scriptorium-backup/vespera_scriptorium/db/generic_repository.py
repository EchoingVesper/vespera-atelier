...
from ..domain.repositories.task_repository import (
    TaskRepository as AbstractTaskRepository,
)
...
from ..orchestrator.generic_models import Task as GenericTask
...
class GenericTaskRepository(TaskRepository):
    ...
    async def get_task(
        self, task_id: str, include_children: bool = False, include_events: bool = False
    ...
    async def get_subtree(
        self, task_id: str, max_depth: Optional[int] = None
    ) -> List[GenericTask]:
    ...
    async def move_task(
        self, task_id: str, new_parent_id: Optional[str], position: Optional[int] = None
    ) -> GenericTask:
    ...
    async def check_dependencies(
        self, task_id: str
    ) -> Tuple[bool, List[TaskDependency]]:
    ...
    async def search_by_attribute(
        self, attribute_name: str, attribute_value: str, indexed_only: bool = True
    ) -> List[GenericTask]:
        """Search tasks by attribute value."""
        from .repository.query_builder import search_by_attribute

        return await search_by_attribute(
            self, attribute_name, attribute_value, indexed_only
        )
    ...
    async def get_parent_task_id(self, task_id: str) -> Optional[str]:
        ...
        try:
            async with self.get_session() as session:
                ...
                logger.debug(
                    f"Retrieved parent task ID for {task_id}: {parent_task_id}"
                )
            ...
            logger.error(
                f"Unexpected error retrieving parent task ID for {task_id}: {e}"
            )
            from ..domain.exceptions.base_exceptions import InfrastructureError
    ...
    async def _save_attribute(
        self, session: AsyncSession, task_id: str, attr: TaskAttribute
    ):
    ...
    async def _record_event(
        ...
        return await record_event(
            session, task_id, event_type, category, triggered_by, data
        )
    ...
    def __init__(self, db_url: Optional[str] = None):
        ...
        if db_url is None:
            ...
            if os.path.exists(".vespera_scriptorium"):
                db_url = f"sqlite:///{workspace_dir}/.vespera_scriptorium/tasks.db"
            else:
    ...
    def list_tasks(
        ...
        return self._sqlite_repo.list_tasks(
            session_id, parent_task_id, status, limit, offset
        )
