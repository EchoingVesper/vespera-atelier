...
class SchemaComparator:
    ...
    def compare_schemas(self) -> SchemaComparisonResult:
        ...
        try:
            ...
            logger.info(
                f"Schema comparison complete: {result.migration_complexity} complexity, "
    ...
    def _compare_columns(
        self, model_columns: Dict, db_columns: Dict, table_diff: TableDifference
    ):
    ...
    def _compare_column_types(
        self, col_name: str, model_col: Column, db_col: Dict
    ...
    def _compare_indexes(
        self, table_name: str, model_table: Table, table_diff: TableDifference
    ):
    ...
    def _has_significant_differences(self, table_diff: TableDifference) -> bool:
        ...
        return (
            bool(table_diff.column_differences)
            or bool(table_diff.index_differences)
            or bool(table_diff.constraint_differences)
        )
    ...
    def _assess_complexity(self, result: SchemaComparisonResult) -> str:
        ...
        table_operations = len(result.tables_needing_creation) + len(
            result.tables_needing_removal
        )
        column_operations = sum(
            len(diff.column_differences) for diff in result.table_differences.values()
        )

        # Check for complex operations
        has_type_changes = any(
            any(
                col_diff.difference_type == "type_mismatch"
    ...
    def _estimate_downtime(self, result: SchemaComparisonResult) -> float:
        ...
        complexity_multiplier = {"SIMPLE": 1.0, "MODERATE": 1.5, "COMPLEX": 3.0}.get(
            result.migration_complexity, 1.0
        )
    ...
    def _generate_safety_warnings(self, result: SchemaComparisonResult) -> List[str]:
        ...
        if result.tables_needing_removal:
            warnings.append(
                f"Tables will be removed: {', '.join(result.tables_needing_removal)}"
            )
        ...
        for table_name, table_diff in result.table_differences.items():
            ...
            if drop_columns:
                warnings.append(
                    f"Columns will be dropped from {table_name}: {', '.join(drop_columns)}"
                )
