...
class WorkspaceMigration:
    ...
    def __init__(self, db_url: Optional[str] = None, base_dir: Optional[str] = None):
        ...
        self.db_url = (
            db_url
            or f"sqlite:///{self.base_dir / '.vespera_scriptorium' / 'workspace.db'}"
        )
    ...
    def _update_existing_tables(self):
        ...
        with self.engine.connect() as conn:
            try:
                ...
                logger.warning(
                    f"Some schema updates may have failed (likely already applied): {e}"
                )
                    ...
                    for op_id, file_path in file_ops:
                        try:
                            ...
                            """
                                ...
                                {"file_path": file_path, "op_id": op_id},
                            )
    ...
    def _create_workspace_configuration(self):
        ...
        default_configs = [
            ...
            ("directories", "logs_dir", ".vespera_scriptorium/logs", "string"),
            ("directories", "temp_dir", ".vespera_scriptorium/temp", "string"),
    ...
    def validate_migration(self) -> Dict[str, Any]:
        ...
        try:
            with self.SessionLocal() as session:
                ...
                task_associations = session.execute(
                    text(
                        """
                ...
                detection_result = session.execute(
                    text(
                        """
...
def run_workspace_migration(
    db_url: Optional[str] = None, base_dir: Optional[str] = None
) -> Dict[str, Any]:
