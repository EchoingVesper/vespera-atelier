...
# Configure logging
logger = logging.getLogger("mcp_vespera_scriptorium.db.migration")
...
class PersistenceMigrator:
    ...
    def migrate_active_tasks(self) -> int:
        ...
        logger.info(
            f"Migration completed: {success_count}/{len(active_task_ids)} tasks migrated successfully"
        )
    ...
    def migrate_archived_tasks(self) -> int:
        ...
        logger.info(
            f"Archive migration completed: {success_count}/{len(archived_task_ids)} tasks migrated successfully"
        )
        return success_count
    ...
    def migrate_all(self) -> int:
        ...
        logger.info(
            f"Total migration completed: {total_count} tasks migrated successfully"
        )
...
def migrate_persistence(
    base_dir: Optional[str] = None, db_url: Optional[str] = None
) -> int:
