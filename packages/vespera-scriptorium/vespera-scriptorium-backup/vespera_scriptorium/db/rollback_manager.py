...
class RollbackManager:
    ...
    def rollback_migration(
        self, migration_id: int, use_backup: bool = False
    ) -> RollbackResult:
        """
        ...
        result = RollbackResult(
            success=False, migration_id=migration_id, execution_time_ms=0
        )
        ...
        try:
            ...
            if result.success:
                ...
                logger.info(
                    f"Rollback completed successfully for migration {migration_id}"
                )
            else:
                logger.error(
                    f"Rollback failed for migration {migration_id}: {result.error_message}"
                )
    ...
    def _rollback_with_sql(
        self, migration_record: Dict[str, Any], result: RollbackResult
    ) -> RollbackResult:
        ...
        try:
            ...
            with self.engine.begin() as conn:
                ...
                statements = [
                    stmt.strip() for stmt in rollback_sql.split(";") if stmt.strip()
                ]
    ...
    def _rollback_with_backup(
        ...
        try:
            ...
            backup_info = self.backup_manager.find_backup_for_migration(
                migration_time, batch_id
            )
            ...
            if success:
                ...
                logger.info(
                    f"Successfully restored database from backup: {backup_info.backup_id}"
                )
            else:
        ...
        try:
            ...
            for migration in migrations:
                ...
                if migration["status"] in ["completed", "failed"]:
                    ...
                    has_backup = (
                        self.backup_manager.find_backup_for_migration(migration_time)
                        is not None
                    )
        ...
        try:
            ...
            if failed_migrations:
                status["recommendations"].append(
                    f"Consider rolling back {len(failed_migrations)} recent failed migrations"
                )

            if backup_stats["total_backups"] == 0:
                status["recommendations"].append(
                    "No backups available - consider enabling automatic backups"
                )

            if backup_stats.get("total_size_mb", 0) > 1000:  # > 1GB
                status["recommendations"].append(
                    "Large backup storage usage - consider cleanup"
                )
