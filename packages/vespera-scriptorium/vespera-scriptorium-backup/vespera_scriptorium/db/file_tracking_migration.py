...
logger = logging.getLogger("mcp_vespera_scriptorium.db.file_tracking_migration")
    ...
    def add_file_tracking_columns_to_subtasks(self) -> bool:
        ...
        try:
            ...
            if "verification_status" not in existing_columns:
                self.engine.execute(
                    "ALTER TABLE subtasks ADD COLUMN verification_status VARCHAR DEFAULT 'pending'"
                )
    ...
    def migrate_metadata_column_rename(self) -> bool:
        ...
        try:
            ...
            if not file_operations_table:
                logger.info(
                    "file_operations table not found, skipping metadata column migration"
                )
                return True
            ...
            if has_old_metadata and not has_new_file_metadata:
                ...
                with self.engine.begin() as conn:
                    ...
                    conn.execute(
                        text(
                            "ALTER TABLE file_operations ADD COLUMN file_metadata JSON"
                        )
                    )
                    conn.execute(
                        text("UPDATE file_operations SET file_metadata = metadata")
                    )
