#!/usr/bin/env python3
"""
Optimized Vespera Scriptorium Server
...
from .reboot.reboot_tools import REBOOT_TOOL_HANDLERS, REBOOT_TOOLS
...
def initialize_database_with_migration(
    base_dir: str = None, db_path: str = None
) -> bool:
    """
    ...
    try:
        ...
        if db_path is None:
            try:
                ...
                if (
                    hasattr(config.paths, "workspace_dir")
                    and config.paths.workspace_dir
                ):
            ...
            if not db_path:
                db_path = os.path.join(
                    base_dir, ".vespera_scriptorium", "vespera_scriptorium.db"
                )
        ...
        if result.success:
            if result.migration_needed:
                logger.info(
                    f"Database migration completed: {result.operations_executed} operations in {result.execution_time_ms}ms"
                )
...
def get_state_manager() -> StateManager:
    ...
    if _state_manager is None:
        ...
        if not migration_success:
            logger.warning(
                "Database migration failed - StateManager may encounter schema issues"
            )
        ...
        logger.info(
            f"Initialized StateManager with persistence in {base_dir}/.vespera_scriptorium"
        )
...
def get_orchestrator(project_dir: str = None) -> TaskOrchestrator:
    ...
    if _orchestrator is None:
        ...
        _orchestrator = TaskOrchestrator(
            state_mgr, specialist_mgr, project_dir=project_dir
        )
...
def get_orchestration_coordinator() -> OrchestrationCoordinator:
    ...
    if not _use_dependency_injection or _di_coordinator is None:
        raise RuntimeError(
            "Dependency injection must be enabled to use OrchestrationCoordinator"
        )
        ...
        types.Tool(
            ...
            inputSchema={
                ...
                "properties": {
                    "working_directory": {
                        "type": "string",
                        "description": "Path where .vespera_scriptorium should be created. If not specified, uses current working directory.",
                    }
                },
            },
        ),
...
async def handle_initialize_session(args: Dict[str, Any]) -> List[types.TextContent]:
    ...
    response = {
        ...
        "orchestrator_path": os.path.join(project_dir, ".vespera_scriptorium"),
        ...
        "active_tasks": [
            {
                ...
                "subtasks": [
                    ...
                    for task in active_tasks
                    if await state_manager._get_parent_task_id(task.task_id)
                    == parent_id
                ],
            }
...
async def handle_plan_task(args: Dict[str, Any]) -> List[types.TextContent]:
    ...
    try:
        ...
        response = {"task_created": False, "error": str(e)}
...
async def handle_execute_subtask(args: Dict[str, Any]) -> List[types.TextContent]:
    ...
    try:
        ...
        return [types.TextContent(type="text", text=specialist_context)]
    except asyncio.TimeoutError:
        ...
        error_response = {"error": str(e), "task_id": task_id}
        return [
            types.TextContent(type="text", text=json.dumps(error_response, indent=2))
        ]
...
async def handle_complete_subtask(args: Dict[str, Any]) -> List[types.TextContent]:
    ...
    try:
        ...
        logger.info(
            f"Completed subtask {task_id} with artifact {artifact_info['artifact_id']}"
        )
        ...
        try:
            ...
            completion_result = await asyncio.wait_for(
                orchestrator.complete_subtask(
                    task_id, summary, legacy_artifacts_list, next_action
                ),
                timeout=30.0,
            )
            ...
            logger.error(
                f"Both artifact and legacy methods failed for task {task_id}: {str(fallback_error)}"
            )
            completion_result = {
                "task_id": task_id,
                "status": "error",
    ...
    return [
        types.TextContent(type="text", text=json.dumps(completion_result, indent=2))
    ]
...
async def handle_synthesize_results(args: Dict[str, Any]) -> List[types.TextContent]:
    ...
    try:
        ...
        error_response = {"error": str(e), "parent_task_id": parent_task_id}
        return [
            types.TextContent(type="text", text=json.dumps(error_response, indent=2))
        ]
...
async def handle_get_status(args: Dict[str, Any]) -> List[types.TextContent]:
    ...
    try:
        ...
        status = await asyncio.wait_for(
            orchestrator.get_status(include_completed), timeout=20  # 20 seconds timeout
        )
...
async def handle_maintenance_coordinator(
    ...
    try:
        ...
        if not hasattr(state_manager, "persistence"):
            raise TypeError(
                f"StateManager object missing 'persistence' attribute. Got type: {type(state_manager)}"
            )

        persistence_manager = state_manager.persistence
        if not hasattr(persistence_manager, "session_scope"):
            raise TypeError(
                f"Persistence manager missing 'session_scope' method. Got type: {type(persistence_manager)}"
            )
        ...
        if action == "scan_cleanup":
            result = await maintenance.scan_and_cleanup(
                scope, validation_level, target_task_id
            )
        elif action == "validate_structure":
            result = await maintenance.validate_structure(
                scope, validation_level, target_task_id
            )
        elif action == "update_documentation":
            result = await maintenance.update_documentation(
                scope, validation_level, target_task_id
            )
        elif action == "prepare_handover":
            result = await maintenance.prepare_handover(
                scope, validation_level, target_task_id
            )
        else:
        ...
        return [
            types.TextContent(
                type="text", text=json.dumps(placeholder_response, indent=2)
            )
        ]
...
async def main():
    ...
    try:
        ...
        if enable_di:
            ...
            logger.info(
                "Dependency injection disabled by configuration, using legacy mode"
            )
            disable_dependency_injection()
        ...
        logger.info("Vespera Scriptorium server shutdown gracefully")
    except Exception as e:
        logger.error(f"Error in Vespera Scriptorium server: {e}", exc_info=True)
        raise
    finally:
        # Cleanup resources
...
def get_project_directory(args: Dict[str, Any]) -> str:
    ...
    try:
        ...
        if hasattr(config.paths, "workspace_dir") and config.paths.workspace_dir:
            if os.path.isdir(config.paths.workspace_dir):
                logger.info(
                    f"Using project directory from configuration: {config.paths.workspace_dir}"
                )
    ...
    editor_cwd = (
        ...
        or os.environ.get("WINDSURF_CWD")
    )
