...
issues in the Vespera Scriptorium tool.
"""
...
class HangDetector:
        ...
        while not self._shutdown:
            try:
                ...
                for op_id, op_info in list(self.active_operations.items()):
                    ...
                    if duration > self.warning_timeout and not op_info.get(
                        "warning_logged"
                    ):
                        logger.warning(
                    ...
                    if duration > self.operation_timeout:
                        ...
                        self.hang_statistics[operation_name] = (
                            self.hang_statistics.get(operation_name, 0) + 1
                        )
    ...
    def register_operation(
        ...
        cancel_token: Optional[asyncio.Task] = None,
    ) -> str:
    ...
    def unregister_operation(self, operation_id: str):
        ...
        if operation_id in self.active_operations:
            ...
            logger.debug(
                f"Unregistered operation {operation_id} ({operation_name}) after {duration:.2f}s"
            )
...
def with_hang_detection(operation_name: str = None, timeout: float = 30.0):
    ...
    def decorator(func: Callable):
        ...
        async def wrapper(*args, **kwargs):
            ...
            op_id = _hang_detector.register_operation(
                operation_name, cancel_token=operation_task
            )
