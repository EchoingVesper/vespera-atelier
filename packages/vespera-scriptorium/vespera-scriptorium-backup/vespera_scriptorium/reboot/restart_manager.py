...
from .state_serializer import RestartReason, ServerStateSnapshot, StateSerializer
...
class ProcessManager:
    ...
    async def start_new_process(
        self, restart_reason: RestartReason, timeout: int = 30
        ...
        try:
            ...
            while time.time() - start_time < timeout:
                ...
                if await self._check_process_health():
                    logger.info(
                        f"New process started successfully: PID {self.current_process.pid}"
                    )
...
class StateRestorer:
    ...
    async def restore_from_snapshot(
        self, snapshot: ServerStateSnapshot, state_manager
    ) -> bool:
        ...
        try:
            ...
            if not await self._restore_database_state(
                snapshot.database_state, state_manager
            ):
                logger.error("Database state restoration failed")
                return False

            # Restore active tasks
            if not await self._restore_active_tasks(
                snapshot.active_tasks, state_manager
            ):
            ...
            if snapshot.suspended_tasks:
                if not await self._restore_suspended_tasks(
                    snapshot.suspended_tasks, state_manager
                ):
    ...
    async def _restore_database_state(self, db_state, state_manager) -> bool:
        ...
        try:
            ...
            current_checksum = self.state_serializer._calculate_db_checksum(
                db_state.db_path
            )
    ...
    async def _restore_active_tasks(
        self, active_tasks: List[Dict[str, Any]], state_manager
    ) -> bool:
        ...
        try:
            ...
            for task_data in active_tasks:
                try:
                    ...
                    logger.error(
                        f"Failed to restore task {task_data.get('task_id', 'unknown')}: {e}"
                    )
    ...
    async def _restore_suspended_tasks(
        self, suspended_tasks: List[Dict[str, Any]], state_manager
    ) -> bool:
    ...
    def _dict_to_task_breakdown(self, task_data: Dict[str, Any]) -> Task:
        ...
        if task_data.get("created_at"):
            try:
                task_breakdown.created_at = datetime.fromisoformat(
...
class RestartCoordinator:
    ...
    def __init__(
        ...
        self.status = RestartStatus(
            phase=RestartPhase.IDLE, progress_percent=0.0, message="Ready for restart"
        )
    ...
    async def execute_restart(
        ...
        try:
            ...
            self._update_status(
                RestartPhase.COMPLETE, 100.0, "Server restart completed successfully"
            )
            ...
            self._update_status(RestartPhase.FAILED, 0.0, f"Restart failed: {e}")
            return False
        finally:
            self._restart_in_progress = False
    ...
    async def _prepare_restart(self):
        ...
        self._update_status(RestartPhase.PREPARING, 10.0, "Preparing for restart")
    ...
    async def _start_new_process(self) -> tuple[bool, Optional[int]]:
        ...
        self._update_status(
            RestartPhase.STARTING_PROCESS, 25.0, "Starting new server process"
        )
    ...
    async def _load_state_snapshot(self) -> Optional[ServerStateSnapshot]:
        ...
        self._update_status(RestartPhase.LOADING_STATE, 40.0, "Loading state snapshot")

        return await self.state_serializer.load_latest_snapshot()

    async def _restore_database_connections(
        self, snapshot: ServerStateSnapshot, state_manager
    ) -> bool:
        """Restore database connections."""
        self._update_status(
            RestartPhase.RESTORING_DATABASE, 60.0, "Restoring database connections"
        )
    ...
    async def _resume_operations(
        self, snapshot: ServerStateSnapshot, state_manager
    ) -> bool:
        """Resume tasks and operations."""
        self._update_status(
            RestartPhase.RESUMING_TASKS, 80.0, "Resuming tasks and operations"
        )
    ...
    async def _enable_client_connections(self):
        ...
        self._update_status(
            RestartPhase.ENABLING_CONNECTIONS, 90.0, "Enabling client connections"
        )
