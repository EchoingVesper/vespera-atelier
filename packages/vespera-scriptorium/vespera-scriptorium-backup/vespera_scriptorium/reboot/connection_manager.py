...
class RequestBuffer:
    ...
    async def buffer_request(self, session_id: str, request: Dict[str, Any]) -> bool:
        ...
        async with self.lock:
            ...
            logger.debug(
                f"Buffered request for session {session_id}: {request.get('method', 'unknown')}"
            )
            return True
    ...
    async def get_buffered_requests(self, session_id: str) -> List[Dict[str, Any]]:
        ...
        async with self.lock:
            ...
            logger.debug(
                f"Retrieved {len(requests)} buffered requests for session {session_id}"
            )
            return requests
    ...
    def get_buffer_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "sessions": {
                session_id: len(buffer) for session_id, buffer in self.buffers.items()
            },
        }
...
class ReconnectionManager:
    ...
    async def initiate_reconnection(
        self, session_id: str, connection_info: ConnectionInfo
    ) -> bool:
        ...
        async with self.lock:
            if session_id in self.pending_reconnections:
                logger.debug(
                    f"Reconnection already in progress for session {session_id}"
                )
                return False
    ...
    async def wait_for_reconnections(
        ...
        try:
            ...
            results = await asyncio.wait_for(
                asyncio.gather(
                    *self.reconnection_tasks.values(), return_exceptions=True
                ),
                timeout=timeout,
            )
            ...
            return {session_id: False for session_id in self.reconnection_tasks.keys()}
    ...
    async def _handle_reconnection(
        ...
        for attempt in range(self.max_reconnect_attempts):
            try:
                logger.debug(
                    f"Reconnection attempt {attempt + 1} for session {session_id}"
                )
                ...
                logger.error(
                    f"Reconnection attempt {attempt + 1} failed for session {session_id}: {e}"
                )
                delay *= self.backoff_multiplier
        ...
        logger.error(
            f"Reconnection failed for session {session_id} after {self.max_reconnect_attempts} attempts"
        )
        return False
...
class ConnectionManager:
    ...
    async def register_connection(
        self, session_id: str, client_name: str, protocol_version: str
    ) -> ConnectionInfo:
    ...
    async def restore_connections(
        self, client_sessions: List[ClientSession]
    ) -> Dict[str, bool]:
        ...
        for client_session in client_sessions:
            try:
                ...
                connection_info = ConnectionInfo(
                    ...
                    client_name=client_session.context_data.get(
                        "client_name", "unknown"
                    ),
                ...
                success = await self.reconnection_manager.initiate_reconnection(
                    client_session.session_id, connection_info
                )
                ...
                logger.error(
                    f"Failed to restore connection {client_session.session_id}: {e}"
                )
    ...
    async def handle_restart_notification(self, restart_in_seconds: int):
        ...
        logger.info(
            f"Sent restart notification to {len(self._notification_handlers)} handlers"
        )
