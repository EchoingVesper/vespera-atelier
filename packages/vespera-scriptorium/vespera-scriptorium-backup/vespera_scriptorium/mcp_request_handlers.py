...
from ..monitoring.hang_detection import (
...
async def handle_complete_subtask_enhanced(
    args: Dict[str, Any], orchestrator
    ...
    try:
        ...
        async with hang_protected_operation("subtask_update", timeout=10.0):
            ...
            subtask = await asyncio.wait_for(
                orchestrator.state.get_subtask(task_id), timeout=5.0
            )
            ...
            await asyncio.wait_for(
                orchestrator.state.update_subtask(subtask), timeout=5.0
            )
        ...
        async with hang_protected_operation("progress_analysis", timeout=20.0):
            ...
            try:
                ...
                if isinstance(parent_progress, Exception):
                    logger.warning(
                        f"Failed to get parent progress: {str(parent_progress)}"
                    )
                    parent_progress = {
                        "progress": "unknown",
...
async def handle_execute_subtask_enhanced(
    args: Dict[str, Any], orchestrator
) -> List[types.TextContent]:
    ...
    try:
        async with hang_protected_operation("get_specialist_context", timeout=20.0):
            specialist_context = await asyncio.wait_for(
                orchestrator.get_specialist_context(task_id), timeout=15.0
            )
...
async def handle_get_hang_statistics(args: Dict[str, Any]) -> List[types.TextContent]:
    ...
    try:
        ...
        response = {
            "monitoring_status": (
                "active" if stats.get("monitoring_active") else "inactive"
            ),
            "statistics": stats,
...
def _generate_recommendations(stats: Dict[str, Any]) -> List[str]:
    ...
    if active_ops > 5:
        recommendations.append(
            f"High number of active operations ({active_ops}) - consider reducing concurrent load"
        )
    ...
    if total_hangs > 10:
        recommendations.append(
            f"Frequent hangs detected ({total_hangs}) - investigate system performance"
        )
    ...
    if lock_timeouts > 0:
        recommendations.append(
            f"Database lock timeouts detected ({lock_timeouts}) - check database performance"
        )

    deadlocks = db_stats.get("total_deadlocks", 0)
    if deadlocks > 0:
        recommendations.append(
            f"Database deadlocks detected ({deadlocks}) - review transaction patterns"
        )

    if not recommendations:
        recommendations.append(
            "System is operating normally with no hanging issues detected"
        )
