...
class ExecuteTaskUseCase:
    ...
    async def get_task_execution_context(
        self, task_id: str
        ...
        try:
            ...
            if validation_errors:
                raise TaskValidationError(
                    f"Task {task_id} not ready for execution", validation_errors
                )
            ...
            instructions = self._generate_execution_instructions(
                task, specialist_context
            )
    ...
    def _validate_task_for_execution(self, task: Task) -> Dict[str, str]:
        ...
        if task.status not in [TaskStatus.PENDING, TaskStatus.ACTIVE]:
            errors["status"] = (
                f"Task must be PENDING or ACTIVE, but is {task.status.value}"
            )
        ...
        if task.dependencies:
            ...
            if incomplete_deps:
                errors["dependencies"] = (
                    f"Blocking dependencies not satisfied: {incomplete_deps}"
                )

        return errors

    def _generate_execution_instructions(
        self, task: Task, specialist_context: Any
    ) -> List[str]:
    ...
    def _check_dependencies_completed(self, task: Task) -> bool:
        ...
        return all(dep.is_satisfied or not dep.is_blocking for dep in task.dependencies)
