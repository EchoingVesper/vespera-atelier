...
class ManageSpecialistsUseCase:
    ...
    async def list_specialists(self) -> List[Specialist]:
        ...
        try:
            ...
            raise OrchestrationError(f"Failed to list specialists: {str(e)}")

    async def find_best_specialist(
        self, task_requirements: List[str]
    ) -> Optional[Specialist]:
        """
    ...
    async def get_specialist_types(self) -> List[str]:
        ...
        try:
            ...
            for specialist in specialists:
                if (
                    specialist.type == SpecialistType.CUSTOM
                    and specialist.name not in types
                ):
            ...
            raise OrchestrationError(f"Failed to get specialist types: {str(e)}")
