...
class CompleteTaskUseCase:
    ...
    async def complete_task_with_artifacts(
        ...
        try:
            ...
            if not task:
                raise TaskValidationError(
                    f"Task {task_id} not found", {"task_id": task_id}
                )

            # Validate completion data
            validation_errors = self._validate_completion_data(completion_data)
            if validation_errors:
                raise TaskValidationError("Invalid completion data", validation_errors)
            ...
            next_steps = self._determine_next_steps(
                completion_data.get("next_action", "complete"), task
            )
            ...
            raise OrchestrationError(
                f"Failed to complete task {task_id}: {str(e)}", {"task_id": task_id}
            )
    ...
    def _validate_completion_data(self, data: Dict[str, Any]) -> Dict[str, str]:
        ...
        if not data.get("next_action"):
            errors["next_action"] = (
                "Next action is required (continue/needs_revision/blocked/complete)"
            )
        ...
        if data.get("next_action") not in valid_actions:
            errors["next_action"] = (
                f"Invalid next_action. Must be one of: {valid_actions}"
            )
        ...
        if data.get("artifact_type"):
            ...
            if data["artifact_type"] not in valid_types:
                errors["artifact_type"] = (
                    f"Invalid artifact_type. Must be one of: {valid_types}"
                )

        return errors

    async def _store_work_artifacts(
        self,
        task_id: str,
        detailed_work: str,
        artifact_type: str,
