"""
Base exception classes for the Vespera Scriptorium.
...
class BaseOrchestrationError(Exception):
    """
    Base exception for all Vespera Scriptorium errors.
...
class InfrastructureError(BaseOrchestrationError):
    ...
    def __init__(
        self, component: str, failure_reason: str, is_recoverable: bool = False
    ):
        recovery = (
            RecoveryStrategy.RETRY if is_recoverable else RecoveryStrategy.RESTART
        )
...
class PerformanceError(BaseOrchestrationError):
    ...
    def __init__(self, metric: str, threshold: float, actual: float, operation: str):
        super().__init__(
            ...
            severity=(
                ErrorSeverity.HIGH if actual > threshold * 2 else ErrorSeverity.MEDIUM
            ),
