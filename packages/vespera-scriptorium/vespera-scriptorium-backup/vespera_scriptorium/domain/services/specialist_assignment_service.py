    ...
    async def get_specialist_context(self, task_id: str) -> str:
        ...
        if role_def:
            context_parts.append(
                f"Role: {role_def.get('name', specialist_type.title())}"
            )
        ...
        self.state_repo.record_event(
            ...
            {"task_id": task_id, "specialist_type": specialist_type},
        )

        return "\n".join(context_parts)

    async def assign_specialist(
        self, task_id: str, specialist_type: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        if not specialist_id:
            specialists = self.specialist_repo.list_specialists(
                category=specialist_type, active_only=True, limit=1
            )
            if specialists:
        ...
        self.specialist_repo.record_specialist_usage(
            specialist_id, task_id, {"assignment_type": "automatic"}
        )

        # Record event
        self.state_repo.record_event(
            task.get("session_id"), "specialist_assigned", assignment
        )
        ...
        for dep_id in dependencies:
            ...
            if dep_task:
                ...
                if status == "completed":
                    ...
                    result_artifacts = [
                        a for a in artifacts if a.get("type") == "result"
                    ]
                    if result_artifacts:
                        context_parts.append(
                            f"  Result: {result_artifacts[0].get('content', 'No content')}"
                        )
