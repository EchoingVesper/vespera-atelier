...
class LifecycleAnalyticsService:
    ...
    async def get_lifecycle_statistics(
        self, time_window_days: int = 30, include_detailed_breakdown: bool = True
    ) -> LifecycleMetrics:
        """
        ...
        """
        logger.info(
            f"Generating lifecycle statistics for {time_window_days} day window"
        )
        ...
        async with self.db_manager.session_scope() as session:
            ...
            # Resource utilization
            specialist_utilization = await self._calculate_specialist_utilization(
                session, time_window_days
            )
            ...
            return LifecycleMetrics(
                ...
                # Performance metrics
                average_completion_time_hours=performance_metrics[
                    "avg_completion_time"
                ],
                task_success_rate=performance_metrics["success_rate"],
                workflow_completion_rate=performance_metrics[
    ...
    async def _get_task_counts(self, session) -> Dict[str, int]:
        ...
        result = await session.execute(
            select(
                SubTaskModel.status, func.count(SubTaskModel.task_id).label("count")
    ...
    async def _calculate_performance_metrics(
        self, session, time_window_days: int
    ) -> Dict[str, float]:
        ...
        completion_time_result = await session.execute(
            select(
                func.avg(
                    func.extract(
                        "epoch", SubTaskModel.completed_at - SubTaskModel.created_at
                    )
                    / 3600
                ).label("avg_hours")
            ).where(
        ...
        task_success_result = await session.execute(
            select(
                func.count(SubTaskModel.task_id)
                .filter(SubTaskModel.status == "completed")
                .label("completed"),
                func.count(SubTaskModel.task_id)
                .filter(SubTaskModel.status.in_(["completed", "failed"]))
                .label("terminal"),
        ...
        success_rate = (
            (success_data.completed / success_data.terminal * 100)
            if success_data.terminal > 0
            else 0.0
        )
    ...
    async def _calculate_workflow_completion_rate(
        self, session, cutoff_date: datetime
    ) -> float:
        ...
        try:
            ...
            workflow_result = await session.execute(
                ...
                .where(TaskBreakdownModel.created_at >= cutoff_date)
            ...
            for workflow in workflows:
                if workflow.total_subtasks > 0:
                    completion_rate = (
                        workflow.completed_subtasks / workflow.total_subtasks
                    )
    ...
    async def _calculate_specialist_utilization(
        self, session, time_window_days: int
    ) -> Dict[str, float]:
        ...
        try:
            ...
            for row in specialist_result:
                if row.total_tasks > 0:
                    utilization[row.specialist_type] = (
    ...
    async def _get_archive_statistics(self, session) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"total_size_mb": total_size_mb, "by_reason": by_reason}
    ...
    async def get_specialist_performance_summary(
        self, specialist_type: str, time_window_days: int = 30
    ) -> Dict[str, Any]:
        ...
        async with self.db_manager.session_scope() as session:
            ...
            result = await session.execute(
                select(
                    ...
                    .label("completed_tasks"),
                    func.count(SubTaskModel.task_id)
                    .filter(SubTaskModel.status == "failed")
                    .label("failed_tasks"),
                    func.avg(
                        func.extract(
                            "epoch", SubTaskModel.completed_at - SubTaskModel.created_at
