...
from ..entities.task import DependencyType, LifecycleStage, Task, TaskType
...
class TaskBreakdownService:
    ...
    async def plan_task(
        ...
        main_task = Task(
            ...
            context=(
                {"original_context": context, "session_id": session_id}
                if context
        ...
        for i, subtask_data in enumerate(subtasks_data):
            subtask = self._create_subtask(
                subtask_data, main_task_id, main_task.hierarchy_path, i
            )
    ...
    async def estimate_effort(self, subtasks: List[Task]) -> Dict[str, Any]:
        ...
        effort_map = {"simple": 1, "moderate": 3, "complex": 8, "very_complex": 21}
        ...
        for subtask in subtasks:
            complexity = (
                ...
                if hasattr(subtask.complexity, "value")
                else str(subtask.complexity)
            )
