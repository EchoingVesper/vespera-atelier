...
from ..repositories import StateRepository, TaskRepository
...
class ResultSynthesisService:
    ...
    def __init__(
        self, task_repository: TaskRepository, state_repository: StateRepository
    ):
        """
    ...
    async def synthesize_task_results(self, parent_task_id: str) -> str:
        ...
        for subtask in subtasks:
            if subtask["status"] in ("completed", "failed"):
                ...
                result_artifacts = [a for a in artifacts if a.get("type") == "result"]

                subtask_info = {
                    "id": subtask["id"],
                    "title": subtask.get("title", "Untitled"),
                    "status": subtask["status"],
                    "specialist": subtask.get("metadata", {}).get(
                        "specialist", "unknown"
                    ),
                    "results": [],
                }
                ...
                code_artifacts = [
                    a for a in artifacts if a.get("type") in ("code", "implementation")
                ]
        ...
        synthesis_parts.append(
            f"# Task Synthesis: {parent_task.get('title', 'Untitled')}"
        )
        synthesis_parts.append("")
    ...
    def _generate_summary(
        self, parent_task: Dict[str, Any], subtask_results: List[Dict[str, Any]]
    ) -> List[str]:
        ...
        all_succeeded = all(
            result["status"] == "completed" for result in subtask_results
        )
        ...
        if all_succeeded:
            ...
            failed_count = sum(
                1 for result in subtask_results if result["status"] == "failed"
            )
        ...
        for specialist, tasks in by_specialist.items():
            summary_parts.append(
                f"- **{specialist.title()}**: {len(tasks)} tasks completed"
            )
    ...
    async def get_partial_synthesis(
        self, parent_task_id: str, include_incomplete: bool = True
    ) -> Dict[str, Any]:
        """
        ...
        for subtask in subtasks:
            status = subtask["status"]
            synthesis_info["subtasks"][status] = (
                synthesis_info["subtasks"].get(status, 0) + 1
            )
        ...
        if synthesis_info["subtasks"]["total"] > 0:
            ...
            synthesis_info["progress_percentage"] = round(
                (completed_and_failed / synthesis_info["subtasks"]["total"]) * 100, 1
            )
        else:
