...
class LifecycleStateMachine:
    ...
    def validate_transition_sequence(cls, stages: List[LifecycleStage]) -> List[str]:
        ...
        for i in range(len(stages) - 1):
            ...
            if not cls.can_transition(current, next_stage):
                errors.append(
                    f"Invalid transition from {current.value} to {next_stage.value}"
                )
    ...
    def suggest_next_stage(
        cls, current_stage: LifecycleStage, context: Dict[str, any] = None
    ) -> LifecycleStage:
        ...
        if context and default_suggestion:
            ...
            if context.get("has_errors") and current_stage == LifecycleStage.ACTIVE:
                ...
                context.get("dependencies_blocked")
                and current_stage == LifecycleStage.READY
            ):
