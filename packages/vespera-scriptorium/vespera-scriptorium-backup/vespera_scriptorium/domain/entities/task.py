...
from ..validation import ValidationError as DomainValidationError
...
from ..value_objects.flexible_specialist_type import (
    validate_specialist_type as validate_specialist_type_func,
)
...
class LifecycleStateMachine:
    ...
    TRANSITIONS = {
        ...
        LifecycleStage.FAILED: [LifecycleStage.ARCHIVED, LifecycleStage.SUPERSEDED],
    ...
    @classmethod
    def can_transition(
        cls, from_stage: LifecycleStage, to_stage: LifecycleStage
    ) -> bool:
    ...
    def get_allowed_transitions(
        cls, current_stage: LifecycleStage
    ) -> List[LifecycleStage]:
...
class TaskAttribute(BaseModel):
    ...
    is_indexed: bool = Field(
        default=False, description="Whether to index for fast lookup"
    )
...
class TaskDependency(BaseModel):
    ...
    is_mandatory: bool = Field(
        default=True, description="Whether dependency must be satisfied"
    )
    auto_satisfy: bool = Field(
        default=False, description="Auto-satisfy when prerequisite completes"
    )
    satisfaction_criteria: Optional[Dict[str, Any]] = Field(
        None, description="Specific criteria"
    )

    # Data dependencies
    output_artifact_ref: Optional[str] = Field(
        None, description="Reference to output artifact"
    )
    input_parameter_name: Optional[str] = Field(
        None, description="Parameter to receive data"
    )
    ...
    def validate_dependency_logic(cls, values):
        ...
        if dep_type == DependencyType.DATA:
            if not values.get("output_artifact_ref") or not values.get(
                "input_parameter_name"
            ):
                raise ValueError(
                    "Data dependencies must specify output_artifact_ref and input_parameter_name"
                )
        ...
        if values.get("waived_at"):
            if not values.get("waived_by") or not values.get("waiver_reason"):
                raise ValueError(
                    "Waived dependencies must have waived_by and waiver_reason"
                )
...
class TaskEvent(BaseModel):
    ...
    event_data: Optional[Dict[str, Any]] = Field(
        None, description="Event-specific data"
    )
...
class TaskArtifact(BaseModel):
    ...
    content_hash: Optional[str] = Field(
        None, description="Content hash for verification"
    )
...
class Task(BaseModel):
    ...
    def validate_specialist_type(cls, v):
        ...
        if v is not None and not validate_specialist_type_func(v):
    ...
    def check_dependencies_satisfied(self) -> Tuple[bool, List[TaskDependency]]:
        ...
        for dep in self.dependencies:
            if dep.is_mandatory and dep.dependency_status not in [
                DependencyStatus.SATISFIED,
                DependencyStatus.WAIVED,
            ]:
    ...
    def to_dict_for_storage(self) -> Dict[str, Any]:
        ...
        data = self.dict(
            exclude={"attributes", "dependencies", "artifacts", "events", "children"}
        )
...
class TaskTemplate(BaseModel):
    ...
    def instantiate(
        self, parameters: Dict[str, Any], parent_task_id: Optional[str] = None
    ) -> List[Task]:
        ...
        def create_tasks_from_structure(
            ...
            for task_key, task_def in structure.items():
                ...
                hierarchy_path = (
                    f"{parent_path}/{task_id}" if parent_path else f"/{task_id}"
                )
                ...
                task = Task(
                    ...
                    title=substitute_params(
                        task_def.get("title", ""), validated_params
                    ),
                    description=substitute_params(
                        task_def.get("description", ""), validated_params
                    ),
...
def create_task_from_subtask(
    subtask: "SubTask", parent_task_id: str, parent_path: str, position: int = 0
) -> Task:
