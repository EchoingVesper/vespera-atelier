...
async def handle_auto_append_create_rule(
    args: Dict[str, Any],
        ...
        try:
            ...
            return [
                types.TextContent(
                    type="text",
                    text=json.dumps(
                        {"status": "error", "error": f"Invalid trigger event: {e}"},
                        indent=2,
                    ),
                )
            ]
        ...
        try:
            ...
            return [
                types.TextContent(
                    type="text",
                    text=json.dumps(
                        {"status": "error", "error": f"Invalid condition format: {e}"},
                        indent=2,
                    ),
                )
            ]
...
async def handle_auto_append_list_rules(
    args: Dict[str, Any],
) -> List[types.TextContent]:
...
async def handle_auto_append_get_rule(args: Dict[str, Any]) -> List[types.TextContent]:
    ...
    try:
        ...
        if not rule_id:
            return [
                types.TextContent(
                    type="text",
                    text=json.dumps(
                        {"status": "error", "error": "rule_id is required"}, indent=2
                    ),
                )
            ]
        ...
        if not rule:
            return [
                types.TextContent(
                    type="text",
                    text=json.dumps(
                        {"status": "error", "error": f"Rule not found: {rule_id}"},
                        indent=2,
                    ),
                )
            ]
        ...
        return [
            types.TextContent(
                type="text",
                text=json.dumps({"status": "success", "rule": rule_data}, indent=2),
            )
        ]
...
async def handle_auto_append_delete_rule(
    args: Dict[str, Any],
) -> List[types.TextContent]:
...
async def handle_auto_append_get_statistics(
    args: Dict[str, Any],
) -> List[types.TextContent]:
...
async def handle_auto_append_test_condition(
    args: Dict[str, Any],
) -> List[types.TextContent]:
    ...
    try:
        ...
        if not condition_data:
            return [
                types.TextContent(
                    type="text",
                    text=json.dumps(
                        {"status": "error", "error": "condition is required"}, indent=2
                    ),
                )
            ]
        ...
        try:
            if "operator" in condition_data and "conditions" in condition_data:
                ...
                from .event_system import EventType, TaskEvent

                mock_event = TaskEvent(
                    event_type=EventType(
                        sample_event_data.get("event_type", "task_completed")
                    ),
                ...
                result = evaluator.evaluate_expression(
                    condition, mock_event, sample_task_data
                )
            else:
                # Single condition
