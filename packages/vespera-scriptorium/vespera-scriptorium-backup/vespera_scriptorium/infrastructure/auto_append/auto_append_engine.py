...
from .condition_evaluator import Condition, ConditionEvaluator, LogicalExpression
from .event_system import EventListener, EventType, TaskEvent, get_event_bus
...
class AutoAppendRule:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "task_definition": (
                self.task_definition.to_dict() if self.task_definition else None
            ),
...
class AutoAppendEngine(EventListener):
    ...
    async def handle_event(self, event: TaskEvent) -> None:
        ...
        logger.info(
            f"Found {len(matching_rules)} matching rules for event {event.event_type.value}"
        )
        ...
        for rule in matching_rules:
            if tasks_created >= self.max_tasks_per_event:
                logger.warning(
                    f"Max tasks per event limit reached: {self.max_tasks_per_event}"
                )
                break
    ...
    def _is_in_cooldown(self, rule: AutoAppendRule) -> bool:
        ...
        cooldown_end = rule.last_execution.replace(
            tzinfo=timezone.utc
        ...
        try:
            ...
            if task_data:
                ...
                self._record_execution(
                    rule, event, success=False, error="No task data generated"
                )
                return False
    ...
    async def _create_direct_task(
        self, rule: AutoAppendRule, event: TaskEvent
    ...
    async def _create_template_task(
        ...
        # Load template
        try:
            template_content = await self.storage_manager.load_template(
                rule.template_id
            )
        ...
        try:
            instantiated = self.template_engine.instantiate_template(
                template, parameters
            )
    ...
    def _apply_substitutions(
        self, data: Dict[str, Any], substitutions: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _check_rate_limits(self) -> bool:
        ...
        self._execution_timestamps = [
            ts for ts in self._execution_timestamps if ts > cutoff
        ]
    ...
    def list_rules(
        self, status: Optional[AutoAppendRuleStatus] = None
    ) -> List[AutoAppendRule]:
        ...
        return {
            ...
            "active_rules": len(
                [
                    ...
                    if r.status == AutoAppendRuleStatus.ACTIVE
                ]
            ),
