...
from .retry_coordinator import RetryPolicy, get_retry_coordinator
...
async def _handle_error(
    ...
    if auto_recover:
        ...
        if recovery_result and recovery_result.success:
            logger.info(
                f"Successfully recovered from {type(error).__name__}: {recovery_result.notes}"
            )
...
def with_error_context(
    component: str, operation: Optional[str] = None, **additional_context
):
    """
        ...
        def wrapper(*args, **kwargs):
            try:
                ...
                if any(isinstance(error, et) for et in error_types):
                    if log_suppressed:
                        logger.warning(
                            f"Suppressed {type(error).__name__} in {func.__qualname__}: {error}"
                        )
...
async def safe_call_async(
    func: Callable, *args, default=None, log_errors=True, **kwargs
):
    """
