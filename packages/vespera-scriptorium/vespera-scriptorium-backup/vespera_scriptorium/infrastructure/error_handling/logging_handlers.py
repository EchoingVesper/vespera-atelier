...
class ErrorLogger:
    ...
    def _log_orchestration_error(
        self, error: BaseOrchestrationError, context: Dict[str, Any]
    ):
        ...
        if error.severity == ErrorSeverity.CRITICAL:
            self.logger.critical(
                f"[{error.error_id}] {error.message}", extra=error_data
            )
    ...
    def _log_generic_error(self, error: Exception, context: Dict[str, Any]):
        ...
        self.logger.error(
            f"Unhandled error: {str(error)}", extra=error_data, exc_info=True
        )
...
class ErrorAggregator:
    ...
    def get_critical_errors(self, limit: int = 50) -> List[ErrorEvent]:
        ...
        critical_events = [
            event for event in self.events if event.severity == "critical"
        ]
...
class ErrorMetrics:
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "error_rate_per_hour": (
                last_hour["total_errors"] if last_hour["total_errors"] > 0 else 0
            ),
        }
