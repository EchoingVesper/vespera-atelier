...
class SystemMonitor:
    ...
    def start_monitoring(self):
        ...
        logger.info(
            f"System monitoring started with {self.collection_interval}s interval"
        )
    ...
    def _collect_system_snapshot(self) -> SystemSnapshot:
        ...
        network_sent_mb = (
            network.bytes_sent - self._initial_network_stats.bytes_sent
        ) / (1024 * 1024)
        network_recv_mb = (
            network.bytes_recv - self._initial_network_stats.bytes_recv
        ) / (1024 * 1024)
    ...
    def _record_system_metrics(self, snapshot: SystemSnapshot):
        ...
        self.metrics_collector.record_gauge(
            "system.cpu_percent", snapshot.cpu_percent, unit="%"
        )
        self.metrics_collector.record_gauge(
            "system.memory_percent", snapshot.memory_percent, unit="%"
        )
        self.metrics_collector.record_gauge(
            "system.memory_available_mb", snapshot.memory_available_mb, unit="MB"
        )
        self.metrics_collector.record_gauge(
            "system.disk_usage_percent", snapshot.disk_usage_percent, unit="%"
        )
        self.metrics_collector.record_gauge(
            "system.disk_free_gb", snapshot.disk_free_gb, unit="GB"
        )
        self.metrics_collector.record_gauge(
            "system.network_sent_mb", snapshot.network_sent_mb, unit="MB"
        )
        self.metrics_collector.record_gauge(
            "system.network_recv_mb", snapshot.network_recv_mb, unit="MB"
        )
        self.metrics_collector.record_gauge(
            "system.process_count", snapshot.process_count, unit="count"
        )
        self.metrics_collector.record_gauge(
            "system.thread_count", snapshot.thread_count, unit="count"
        )
        self.metrics_collector.record_gauge(
            "system.file_descriptors", snapshot.file_descriptors, unit="count"
        )
    ...
    def _check_alert_rules(self):
        ...
        for rule in self.alert_rules:
            ...
            metric_summary = self.metrics_collector.get_metric_summary(
                rule.metric_name, timedelta(seconds=rule.duration_seconds)
            )
            ...
            if threshold_exceeded and not alert_state["active"]:
                ...
                duration_exceeded = (
                    datetime.utcnow() - alert_state["triggered_at"]
    ...
    def _trigger_alert(self, rule: AlertRule, current_value: float):
        """Trigger an alert."""
        logger.warning(
            f"ALERT TRIGGERED: {rule.name} - {rule.metric_name} = {current_value} {rule.operator} {rule.threshold}"
        )
    ...
    def _resolve_alert(self, rule: AlertRule, current_value: float):
        """Resolve an alert."""
        logger.info(
            f"ALERT RESOLVED: {rule.name} - {rule.metric_name} = {current_value}"
        )
        self.metrics_collector.increment_counter(f"alerts.resolved.{rule.name}")

    def _default_alert_callback(
        self, alert_name: str, current_value: float, threshold: float
    ):
        """Default alert callback that just logs."""
        logger.critical(
            f"System alert: {alert_name} - Current: {current_value}, Threshold: {threshold}"
        )
                ...
                "rules": [
                    {
                        ...
                        "active": self.alert_states.get(rule.name, {}).get(
                            "active", False
                        ),
                    }
                    for rule in self.alert_rules
