...
class DiagnosticRunner:
    ...
    async def _run_diagnostic(
        self, name: str, diagnostic_func: Callable
    ) -> DiagnosticResult:
    ...
    async def _diagnose_system_info(self) -> DiagnosticResult:
        ...
        try:
            ...
            system_info = SystemInfo(
                ...
                environment_vars={
                    k: v for k, v in os.environ.items() if "SECRET" not in k.upper()
                },
                installed_packages=[],
            )
            ...
            if system_info.memory_usage_mb > 500:
                recommendations.append(
                    "High memory usage detected, consider monitoring for leaks"
                )
    ...
    async def _diagnose_database_integrity(self) -> DiagnosticResult:
        ...
        try:
            ...
            for db_file in db_files:
                try:
                    ...
                    if integrity_result != "ok":
                        issues.append(
                            f"Database {db_file} has integrity issues: {integrity_result}"
                        )
            ...
            return DiagnosticResult(
                ...
                details=details,
                recommendations=(
                    ["Run database repair if issues found"] if issues else []
                ),
                timestamp=datetime.utcnow(),
            )
    ...
    async def _diagnose_file_permissions(self) -> DiagnosticResult:
        ...
        try:
            ...
            important_dirs = [".", ".vespera_scriptorium", "logs", "config"]
            ...
            for dir_path in important_dirs:
                ...
                if path.exists():
                    ...
                    if path.name == "config" and os.access(
                        path, os.R_OK | os.W_OK | os.X_OK
                    ):
                        config_files = list(path.glob("*.yaml")) + list(
                            path.glob("*.json")
                        )
                        for config_file in config_files:
                            if (
                                os.stat(config_file).st_mode & 0o077
    ...
    async def _diagnose_dependencies(self) -> DiagnosticResult:
        ...
        try:
            ...
            if missing_core:
                issues.extend(
                    [f"Missing core dependency: {dep}" for dep in missing_core]
                )

            status = (
                "error" if missing_core else ("warning" if missing_optional else "pass")
            )
    ...
    async def _diagnose_error_patterns(self) -> DiagnosticResult:
        ...
        try:
            ...
            if frequent_errors and frequent_errors[0]["count"] > 5:
                ...
                issues.append(
                    f"Frequent error pattern: {top_error['error_type']} ({top_error['count']} occurrences)"
                )
                recommendations.append(
                    f"Focus on resolving {top_error['error_type']} errors"
                )
        ...
        try:
            if not self.system_monitor:
                return DiagnosticResult(
                    ...
                    details={},
                    recommendations=[
                        "Enable system monitoring for performance analysis"
                    ],
                    timestamp=datetime.utcnow(),
                )
            ...
            if snapshot["disk_usage_percent"] > 90:
                issues.append(
                    f"Low disk space: {snapshot['disk_usage_percent']:.1f}% used"
                )
                recommendations.append("Clean up disk space")

            if snapshot["file_descriptors"] > 500:
                issues.append(
                    f"High file descriptor count: {snapshot['file_descriptors']}"
                )
            ...
            if "all_time" in metrics_report:
                ...
                if "system.memory_percent" in all_time_metrics:
                    ...
                    if memory_stats["max"] - memory_stats["min"] > 20:
                        recommendations.append(
                            "Memory usage varies significantly, check for leaks"
                        )
    ...
    def generate_diagnostic_report(self, results: Dict[str, DiagnosticResult]) -> str:
        ...
        report_lines = [
            "# Vespera Scriptorium - Diagnostic Report",
        ...
        for category, category_results in categories.items():
            report_lines.extend([f"## {category.title()} Diagnostics", ""])
