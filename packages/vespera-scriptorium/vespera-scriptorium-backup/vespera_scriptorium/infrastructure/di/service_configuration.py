...
from ...domain.repositories import SpecialistRepository, StateRepository, TaskRepository
...
def configure_infrastructure_services(
    registrar: ServiceRegistrar, config: Dict[str, Any]
) -> ServiceRegistrar:
    """
    ...
    # Repository Factory
    def create_repository_factory_instance(
    ...
    registrar.register_factory(
        RepositoryFactory, create_repository_factory_instance
    ).as_singleton()
    ...
    def create_specialist_repository(
        container: ServiceContainer,
    ...
    registrar.register_factory(
        SpecialistRepository, create_specialist_repository
    ).as_singleton()

    return registrar


def configure_domain_services(
    registrar: ServiceRegistrar, config: Dict[str, Any]
) -> ServiceRegistrar:
    """
    ...
    registrar.register_factory(
        TaskBreakdownService, create_breakdown_service
    ).as_singleton()
    ...
    registrar.register_factory(
        SpecialistAssignmentService, create_assignment_service
    ).as_singleton()
    ...
    registrar.register_factory(
        ProgressTrackingService, create_tracking_service
    ).as_singleton()
    ...
    registrar.register_factory(
        ResultSynthesisService, create_synthesis_service
    ).as_singleton()
    ...
    registrar.register_factory(
        OrchestrationCoordinator, create_coordinator
    ).as_singleton()

    return registrar


def configure_legacy_services(
    registrar: ServiceRegistrar, config: Dict[str, Any]
) -> ServiceRegistrar:
    """
