...
class AsyncSQLiteSpecialistRepository(SpecialistRepository):
                ...
                """,
                    {
                        ...
                        "capabilities": json.dumps(
                            specialist_data.get("capabilities", [])
                        ),
    ...
    async def get_specialist(self, specialist_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            result = await self.db_adapter.execute_one(
                "SELECT * FROM specialists WHERE id = ?", {"id": specialist_id}
            )
    ...
    async def update_specialist(
        self, specialist_id: str, update_data: Dict[str, Any]
    ) -> bool:
    ...
    async def delete_specialist(self, specialist_id: str) -> bool:
        ...
        try:
            async with self.db_adapter.transaction() as tx:
                ...
                await self.db_adapter.execute(
                    "DELETE FROM specialists WHERE id = ?", {"id": specialist_id}
                )
    ...
    async def assign_specialist(
        self, specialist_id: str, task_id: str, session_id: Optional[str] = None
    ) -> str:
        ...
        try:
            ...
            logger.error(
                f"Failed to assign specialist {specialist_id} to task {task_id}: {e}"
            )
            raise

    async def complete_assignment(
        self, assignment_id: str, result: Optional[str] = None
    ) -> bool:
