...
class LegacyDatabaseAdapter:
    ...
    async def create_task_async(self, task: GenericTask) -> GenericTask:
        ...
        task_id = await asyncio.to_thread(self.task_repository.create_task, task_data)
    ...
    async def get_task_async(self, task_id: str) -> Optional[GenericTask]:
        ...
        """
        task_data = await asyncio.to_thread(self.task_repository.get_task, task_id)
    ...
    async def update_task_status_async(self, task_id: str, status: TaskStatus) -> bool:
        ...
        return await asyncio.to_thread(
            self.task_repository.update_task_status, task_id, status_value
        )
    ...
    async def get_subtasks_async(self, parent_task_id: str) -> List[GenericTask]:
        ...
        task_datas = await asyncio.to_thread(
            self.task_repository.get_subtasks, parent_task_id
        )
    ...
    async def save_session_async(
        ...
        return await asyncio.to_thread(
            self.state_repository.save_session, session_id, session_data
        )

    async def get_session_async(self, session_id: str) -> Optional[Dict[str, Any]]:
        """Async wrapper for getting session."""
        return await asyncio.to_thread(self.state_repository.get_session, session_id)

    async def save_context_async(
        self, session_id: str, context_key: str, context_data: Dict[str, Any]
    ) -> bool:
        """Async wrapper for saving context."""
        return await asyncio.to_thread(
            self.state_repository.save_context, session_id, context_key, context_data
        )
