...
class SQLiteStateRepository(StateRepository):
    ...
    def get_session(self, session_id: str) -> Optional[Dict[str, Any]]:
        ...
        if row:
            session = dict(row)
            session["metadata"] = (
                json.loads(session["metadata"]) if session["metadata"] else {}
            )
            return session
        return None
    ...
    def get_context(
        self, session_id: str, context_key: str
    ...
    def get_workflow_state(self, session_id: str) -> Optional[Dict[str, Any]]:
        ...
        row = self.connection_manager.execute_one(
            "SELECT state_data FROM workflow_states WHERE session_id = ?", (session_id,)
        )
    ...
    def record_event(
        ...
        with self.connection_manager.transaction() as conn:
            ...
            """,
                (event_id, session_id, event_type, json.dumps(event_data), now),
            )
    ...
    def cleanup_old_sessions(
        self, older_than: datetime, keep_active: bool = True
    ) -> int:
