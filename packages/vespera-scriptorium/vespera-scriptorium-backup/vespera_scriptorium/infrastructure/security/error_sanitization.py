"""
Error Sanitization Framework for Vespera Scriptorium
    ...
    def to_mcp_error(self) -> Dict[str, Any]:
        ...
        try:
            ...
            mcp_code = ERROR_CATEGORY_TO_MCP_CODE.get(
                self.category, MCPErrorCode.INTERNAL_ERROR
            )
...
class ErrorSanitizer:
    ...
    ERROR_MAPPINGS = {
        ...
        "DatabaseError": (
            ...
            ["Please try again later", "Check that the system is accessible"],
        ),
        ...
        "ConnectionError": (
            ...
            "Connection failed",
            ["Check your network connection", "Try again in a few moments"],
        ),
        "TimeoutError": (
            ErrorCategory.NETWORK_ERROR,
            "Operation timed out",
        ...
        "ImportError": (
            ...
            ["A required component is not available", "Contact support for assistance"],
        ),
        "AttributeError": (
            ErrorCategory.CONFIGURATION_ERROR,
            "System configuration error",
    ...
    def sanitize_exception(
        ...
        safe_response = SafeErrorResponse(
            ...
            error_id=error_id,
            details=(
                {"sanitized_message": sanitized_details} if sanitized_details else {}
            ),
    ...
    def _generate_error_id(
        ...
        error_content = (
            f"{type(exception).__name__}:{str(exception)}:{self._error_counter}"
        )
        if context:
