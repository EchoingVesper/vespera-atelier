...
from .protocol_adapters import MCPErrorAdapter, MCPRequestAdapter, MCPResponseAdapter
...
class MCPToolHandler:
    ...
    async def handle_plan_task(self, args: Dict[str, Any]) -> List[types.TextContent]:
        ...
        try:
            ...
            error = MCPRequestAdapter.validate_args(
                args, ["description", "subtasks_json"]
            )
            if error:
    ...
    async def handle_execute_subtask(
        self, args: Dict[str, Any]
    ) -> List[types.TextContent]:
    ...
    async def handle_complete_subtask(
        self, args: Dict[str, Any]
    ) -> List[types.TextContent]:
        ...
        try:
            # Validate required arguments
            error = MCPRequestAdapter.validate_args(
                args, ["task_id", "summary", "next_action"]
            )
            if error:
                return MCPErrorAdapter.validation_error(error)
    ...
    async def handle_synthesize_results(
        self, args: Dict[str, Any]
    ) -> List[types.TextContent]:
