...
from ...error_handling.decorators import ErrorContext, handle_errors
...
@handle_errors(
    component="MCPHandler", operation="create_task", auto_retry=True, log_errors=True
)
async def handle_create_task_v2(args: Dict[str, Any]) -> List[types.TextContent]:
    ...
    try:
        ...
        return [types.TextContent(type="text", text=error_response.json(indent=2))]
        ...
        error_response = MCPErrorResponse.from_exception(
            ...
            operation="create_task",
            recovery_suggestions=[
                "Verify the Vespera Scriptorium service is running",
...
@handle_errors(
    component="MCPHandler", operation="update_task", auto_retry=True, log_errors=True
)
...
async def handle_delete_task_v2(args: Dict[str, Any]) -> List[types.TextContent]:
    ...
    try:
        ...
        logger.info(
            f"Deleting task: {request.task_id} (force={request.force}, archive={request.archive_instead})"
        )
        ...
        deletion_result = await use_case.delete_task(
            request.task_id, request.force, request.archive_instead
        )
...
@handle_errors(
    component="MCPHandler", operation="cancel_task", auto_retry=False, log_errors=True
)
async def handle_cancel_task_v2(args: Dict[str, Any]) -> List[types.TextContent]:
    ...
    try:
        ...
        cancellation_result = await use_case.cancel_task(
            request.task_id, request.reason, request.preserve_work
        )
...
@handle_errors(
    component="MCPHandler", operation="query_tasks", auto_retry=True, log_errors=True
)
    ...
    try:
        ...
        for task in query_result["tasks"]:
            ...
            task_result = TaskQueryResult(
                ...
                dependency_ids=(
                    [d.task_id for d in task.dependencies] if task.dependencies else []
                ),
                tags=task.tags,
...
@handle_errors(
    component="MCPHandler", operation="execute_task", auto_retry=True, log_errors=True
)
...
async def handle_complete_task_v2(args: Dict[str, Any]) -> List[types.TextContent]:
    ...
    try:
        ...
        completion_response = await use_case.complete_task_with_artifacts(
            request.task_id, request.dict(exclude_unset=True)
        )
            ...
            next_steps=[
                ...
                NextStep(
                    ...
                    tool_name=(
                        ...
                        if completion_response.triggered_tasks
                        else None
                    ),
                ),
            ],
