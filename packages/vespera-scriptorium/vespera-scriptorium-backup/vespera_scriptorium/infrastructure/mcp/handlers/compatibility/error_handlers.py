...
class ErrorHandlingMixin:
    ...
    def validate_task_exists(
        self, task_id: str, task_data: Optional[Dict[str, Any]]
    ) -> Dict[str, Any]:
        ...
        for field in required_fields:
            if field not in task_data:
                raise ValidationError(
                    field, None, f"Required field missing from task {task_id}"
                )

        return task_data

    def validate_task_state(
        self, task_id: str, current_status: str, required_statuses: List[str]
    ) -> None:
