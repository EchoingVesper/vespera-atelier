...
from ..security.error_sanitization import (
    ErrorCategory,
    SafeErrorResponse,
    sanitize_error,
)
...
def mcp_error_handler(
    tool_name: Optional[str] = None, require_auth: bool = False, log_errors: bool = True
) -> Callable:
    """
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def wrapper(*args, **kwargs) -> List[types.TextContent]:
            ...
            try:
                ...
                if log_errors:
                    logger.debug(
                        f"MCP tool completed: {handler_name} ({duration:.3f}s)"
                    )
                ...
                try:
                    ...
                    return [types.TextContent(type="text", text=mcp_response.to_json())]
                    ...
                    return [
                        types.TextContent(type="text", text=fallback_response.to_json())
                    ]
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def wrapper(*args, **kwargs) -> List[types.TextContent]:
            ...
            for field in required_fields:
                ...
                elif not tool_args[field] or (
                    isinstance(tool_args[field], str) and not tool_args[field].strip()
                ):
            ...
            if missing_fields or invalid_fields:
                ...
                if invalid_fields:
                    error_details.append(
                        f"Empty or invalid fields: {', '.join(invalid_fields)}"
                    )
                ...
                return [types.TextContent(type="text", text=validation_error.to_json())]
...
def format_mcp_error_response(
    ...
    if error_code:
        mcp_response = MCPErrorResponse(
            code=error_code, message=str(error), data=context or {}
        )
    else:
...
def handle_authentication_error(
    ...
    return [types.TextContent(type="text", text=auth_error.to_json())]


def handle_permission_error(action: str, resource: str = "") -> List[types.TextContent]:
    """
    ...
    return [types.TextContent(type="text", text=perm_error.to_json())]
...
def handle_not_found_error(
    ...
    return [types.TextContent(type="text", text=not_found_error.to_json())]
...
def validate_tool_arguments(
    args: Dict[str, Any], required: List[str], optional: Optional[List[str]] = None
) -> Dict[str, Any]:
    """
