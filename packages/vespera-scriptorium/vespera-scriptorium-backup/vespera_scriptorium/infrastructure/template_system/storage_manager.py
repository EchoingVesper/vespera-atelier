...
Manages template storage in the .vespera_scriptorium/templates/ directory
...
class TemplateStorageManager:
    ...
    def __init__(self, workspace_dir: Optional[Path] = None, create_dirs: bool = True):
        ...
        self.orchestrator_dir = self.workspace_dir / ".vespera_scriptorium"
    ...
    def save_template(
        ...
        try:
            ...
            template_with_metadata = self._add_storage_metadata(
                template_data, template_id, category
            )
            ...
            self._update_metadata_registry(
                template_id, template_with_metadata, category
            )
    ...
    def backup_templates(self, backup_dir: Optional[Path] = None) -> Path:
        ...
        if not backup_dir:
            backup_dir = (
                self.workspace_dir
                / "backups"
                / f"templates_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            )
    ...
    def restore_templates(self, backup_dir: Path, overwrite: bool = False) -> None:
        ...
        if not backup_templates_dir.exists():
            raise TemplateStorageError(
                f"Backup templates directory not found: {backup_templates_dir}"
            )
        ...
        try:
            ...
            shutil.copytree(
                backup_templates_dir, self.templates_dir, dirs_exist_ok=not overwrite
            )
    ...
    def get_template_info(
        ...
        try:
            ...
            return {
                ...
                "content_hash": self.security_validator.generate_content_hash(
                    template_data
                ),
            }
            ...
            raise TemplateStorageError(
                f"Failed to get template info for {template_id}: {e}"
            )
    ...
    def _validate_template_id(self, template_id: str) -> bool:
        ...
        return (
            bool(re.match(r"^[a-zA-Z0-9_-]+$", template_id)) and len(template_id) <= 100
        )
    ...
    def _add_storage_metadata(
        self, template_data: Dict[str, Any], template_id: str, category: str
    ) -> Dict[str, Any]:
    ...
    def _save_template_file(
        self, template_file: Path, template_data: Dict[str, Any]
    ) -> None:
...
"""
    ...
    def _update_metadata_registry(
        self, template_id: str, template_data: Dict[str, Any], category: str
    ) -> None:
