...
class TemplateSecurityValidator:
        ...
        if isinstance(value, str):
            if self._contains_dangerous_patterns(value):
                raise SecurityValidationError(
                    f"Parameter {param_name} contains dangerous patterns"
                )
        elif isinstance(value, (list, tuple)):
            for item in value:
                if isinstance(item, str) and self._contains_dangerous_patterns(item):
                    raise SecurityValidationError(
                        f"Parameter {param_name} array contains dangerous patterns"
                    )
        elif isinstance(value, dict):
            for key, val in value.items():
                if isinstance(key, str) and self._contains_dangerous_patterns(key):
                    raise SecurityValidationError(
                        f"Parameter {param_name} object key contains dangerous patterns"
                    )
                if isinstance(val, str) and self._contains_dangerous_patterns(val):
                    raise SecurityValidationError(
                        f"Parameter {param_name} object value contains dangerous patterns"
                    )
    ...
    def _validate_structure_security(self, data: Any, depth: int = 0) -> None:
        ...
        if depth > self.max_nesting_depth:
            raise SecurityValidationError(
                f"Nesting depth exceeds limit: {self.max_nesting_depth}"
            )

        if isinstance(data, dict):
            if len(data) > self.max_object_keys:
                raise SecurityValidationError(
                    f"Object has too many keys: {len(data)} > {self.max_object_keys}"
                )

            for key, value in data.items():
                if not isinstance(key, str):
                    raise SecurityValidationError(
                        f"Object key must be string, got {type(key)}"
                    )

                if len(key) > 1000:
                    raise SecurityValidationError(
                        f"Object key too long: {len(key)} characters"
                    )

                if self._contains_dangerous_patterns(key):
                    raise SecurityValidationError(
                        f"Object key contains dangerous patterns: {key}"
                    )
            ...
            if len(data) > self.max_array_size:
                raise SecurityValidationError(
                    f"Array too large: {len(data)} > {self.max_array_size}"
                )
            ...
            if len(data) > self.max_string_length:
                raise SecurityValidationError(
                    f"String too long: {len(data)} > {self.max_string_length}"
                )
    ...
    def _validate_content_security(self, data: Any) -> None:
        ...
        if isinstance(data, str):
            if self._contains_dangerous_patterns(data):
                raise SecurityValidationError(
                    f"Content contains dangerous patterns: {data[:100]}..."
                )
            ...
            for key, value in data.items():
                if self._contains_dangerous_patterns(key):
                    raise SecurityValidationError(
                        f"Dictionary key contains dangerous patterns: {key}"
                    )
    ...
    def _validate_template_security(self, template: Dict[str, Any]) -> None:
        ...
        if len(template_str) > self.max_template_size:
            raise SecurityValidationError(
                f"Template too large: {len(template_str)} > {self.max_template_size}"
            )
        ...
        for key in metadata:
            if key.lower() not in safe_metadata and any(
                term in key.lower() for term in suspicious_metadata
            ):
    ...
    def _validate_parameter_definition_security(
        self, param_name: str, param_def: Any
    ) -> None:
        ...
        if isinstance(param_def, dict):
            ...
            if pattern:
                try:
                    ...
                    raise SecurityValidationError(
                        f"Invalid regex pattern for parameter {param_name}: {e}"
                    )
                ...
                for dangerous in dangerous_regex:
                    if re.search(dangerous, pattern):
                        logger.warning(
                            f"Potentially dangerous regex pattern in parameter {param_name}: {pattern}"
                        )
