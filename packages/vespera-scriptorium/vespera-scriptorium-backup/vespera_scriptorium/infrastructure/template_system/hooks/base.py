...
class HookContext:
    ...
    def add_spawned_agent(
        self, agent_id: str, agent_type: str, workspace_path: str
    ) -> None:
    ...
    def create_checkpoint(self, checkpoint_name: str) -> None:
        ...
        logger.info(
            f"Created checkpoint '{checkpoint_name}' for execution {self.execution_id}"
        )
...
class HookResult:
    ...
    def add_artifact(self, artifact_path: str, description: str = "") -> None:
        ...
        self.metadata.setdefault("artifacts_created", []).append(
            {"path": artifact_path, "description": description}
        )
...
class ExecutiveDysfunctionHook(Hook):
    ...
    def __init__(
        self, hook_id: str, description: str = "", ed_features: Dict[str, bool] = None
    ):
