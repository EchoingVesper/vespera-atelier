...
class GitBranchHook(ExecutiveDysfunctionHook):
    ...
    async def execute(self, context: HookContext) -> HookResult:
        ...
        try:
            ...
            if self._branch_exists(branch_name, context.workspace_path):
                ...
                branch_name = self._create_unique_branch_name(
                    branch_name, context.workspace_path
                )

            # Create and switch to branch
            success = await self._create_and_checkout_branch(
                branch_name, context.workspace_path
            )
    ...
    async def _create_and_checkout_branch(
        self, branch_name: str, workspace_path: str
    ) -> bool:
        ...
        try:
            ...
            if create_result.returncode == 0:
                ...
                logger.error(
                    f"Failed to create branch {branch_name}: {stderr.decode()}"
                )
                return False
...
class AgentSpawningHook(ExecutiveDysfunctionHook):
    ...
    def _analyze_agent_requirements(
        self, phase_config: Dict[str, Any], context: HookContext
    ) -> List[Dict[str, Any]]:
    ...
    async def _spawn_agent(
        self, agent_spec: Dict[str, Any], context: HookContext
    ) -> Dict[str, Any]:
...
class CheckpointHook(ExecutiveDysfunctionHook):
    ...
    async def execute(self, context: HookContext) -> HookResult:
        ...
        try:
            checkpoint_name = (
                f"phase_{context.current_phase}_{datetime.now().timestamp()}"
            )
            ...
            checkpoint_file = (
                Path(context.workspace_path) / "checkpoints" / f"{checkpoint_name}.json"
            )
...
class ValidationHook(Hook):
    ...
    async def execute(self, context: HookContext) -> HookResult:
        ...
        try:
            ...
            failed_validations = [
                name for name, valid in validation_results if not valid
            ]
...
class CommitHook(ExecutiveDysfunctionHook):
    ...
    def __init__(self):
        super().__init__(
            ...
            ed_features={"reduces_decisions": True, "preserves_momentum": True},
        )
    ...
    async def execute(self, context: HookContext) -> HookResult:
        ...
        try:
            ...
            success = await self._stage_and_commit(
                commit_message, context.workspace_path
            )
    ...
    def _generate_commit_message(self, context: HookContext) -> str:
        ...
        return (
            f"feat({context.template_id}): {phase} phase progress\n\n"
...
class NotificationHook(Hook):
    ...
    def __init__(self):
        super().__init__(
            hook_id="progress_notification", description="Send progress notifications"
        )
    ...
    async def execute(self, context: HookContext) -> HookResult:
        ...
        try:
            ...
            notification_content = {
                ...
                "duration": (
                    f"{execution_duration:.1f}s" if execution_duration else "Unknown"
                ),
...
class DocumentAssociationHook(ExecutiveDysfunctionHook):
    ...
    async def _discover_relevant_documents(self, context: HookContext) -> List[str]:
        ...
        if "implementation" in context.template_id:
            relevant_docs.extend(
                ["docs/architecture/*.md", "docs/api/*.md", "examples/*.py"]
            )

        return relevant_docs

    def _rank_documents_by_relevance(
        self, documents: List[str], context: HookContext
    ) -> List[Dict[str, Any]]:
        ...
        for doc_path in documents:
            ...
            ranked_docs.append({"path": doc_path, "relevance_score": relevance_score})
    ...
    def _calculate_relevance_score(self, doc_path: str, context: HookContext) -> float:
        ...
        if any(
            name in path_lower for name in ["readme", "architecture", "api", "guide"]
        ):
            score += 0.3
