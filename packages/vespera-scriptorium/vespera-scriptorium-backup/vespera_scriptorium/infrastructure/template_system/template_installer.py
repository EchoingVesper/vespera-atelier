...
class TemplateInstaller:
    ...
    async def install_default_library(
        self, category: str = "all", overwrite: bool = False
    ) -> Dict[str, Any]:
        """
        ...
        if category == "all":
            ...
            templates = {
                name: get_all_default_templates()[name]
                for name in template_names
                if name in get_all_default_templates()
            }
            ...
            templates = {
                name: additional_templates[name]
                for name in template_names
                if name in additional_templates
            }
            ...
            try:
                ...
                if result["status"] == "installed":
                    ...
                    results["errors"].append(
                        f"{template_id}: {result.get('error', 'Unknown error')}"
                    )
        ...
        if results["failed"]:
            if results["installed"]:
                ...
                results["message"] = (
                    f"Installed {len(results['installed'])} templates, {len(results['failed'])} failed"
                )
            else:
                results["status"] = "failure"
                results["message"] = (
                    f"Failed to install all {len(results['failed'])} templates"
                )
        else:
            results["message"] = (
                f"Successfully installed {len(results['installed'])} templates"
            )

        logger.info(f"Template installation complete: {results['message']}")
        return results

    async def _install_single_template(
        self,
        template_id: str,
        ...
        try:
            ...
            try:
                ...
                return {"status": "failed", "error": f"JSON5 parsing failed: {str(e)}"}
    ...
    async def install_custom_template(
        self,
        template_id: str,
        template_content: str,
        category: str = "user",
        ...
        if result["status"] == "installed":
            ...
            logger.warning(
                f"Failed to install custom template {template_id}: {result.get('error')}"
            )

        return result

    async def update_template(
        self, template_id: str, template_content: str, category: str = "user"
    ) -> Dict[str, Any]:
        """
    ...
    async def uninstall_template(
        self, template_id: str, category: str = "user"
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            return {
                ...
                "installation_coverage": {
                    ...
                    "self_development": self._calculate_coverage(
                        "self_development", installed
                    ),
                },
            }
    ...
    def _calculate_coverage(
        self, category: str, installed: Dict[str, List[str]]
    ) -> Dict[str, Any]:
    ...
    async def validate_all_templates(
        ...
        try:
            ...
            results = {"status": "success", "validated": [], "failed": [], "errors": []}
                ...
                try:
                    ...
                    for template_info in template_list:
                        ...
                        try:
                            ...
                            parsed_template = self.storage_manager.load_template(
                                template_id, cat
                            )
                            self.security_validator.validate_template(parsed_template)
            ...
            results["message"] = (
                f"Validated {len(results['validated'])} templates, {len(results['failed'])} failed"
            )
