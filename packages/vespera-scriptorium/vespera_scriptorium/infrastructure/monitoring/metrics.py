...
class PerformanceTracker:
    ...
    def __enter__(self):
        ...
        self.metrics_collector.increment_counter(
            f"{self.operation_name}.started", tags=self.tags
        )
        return self
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        self.metrics_collector.record_timing(
            f"{self.operation_name}.duration", duration, self.tags
        )

        if exc_type is None:
            self.metrics_collector.increment_counter(
                f"{self.operation_name}.success", tags=self.tags
            )
        else:
            self.metrics_collector.increment_counter(
                f"{self.operation_name}.error", tags=self.tags
            )
            self.metrics_collector.increment_counter(
                f"{self.operation_name}.error.{exc_type.__name__}", tags=self.tags
            )
...
def record_timing(
    name: str, duration_seconds: float, tags: Optional[Dict[str, str]] = None
):
