...
class EventBus:
    ...
    def subscribe(self, listener: EventListener) -> None:
        ...
        for event_type in supported_events:
            ...
            if listener not in self._listeners[event_type]:
                ...
                logger.debug(
                    f"Subscribed listener {listener.__class__.__name__} to {event_type.value}"
                )
    ...
    def unsubscribe(self, listener: EventListener) -> None:
        ...
        for event_type in list(self._listeners.keys()):
            if listener in self._listeners[event_type]:
                ...
                logger.debug(
                    f"Unsubscribed listener {listener.__class__.__name__} from {event_type.value}"
                )
    ...
    async def _handle_event(self, event: TaskEvent) -> None:
        ...
        logger.info(
            f"Processing event {event.event_type.value} for task {event.task_id}"
        )
    ...
    async def _safe_handle_listener(
        self, listener: EventListener, event: TaskEvent
    ) -> None:
        """Safely handle event for a single listener with error recovery."""
        try:
            await listener.handle_event(event)
            logger.debug(
                f"Listener {listener.__class__.__name__} handled event {event.event_type.value}"
            )
        except Exception as e:
            logger.error(
                f"Listener {listener.__class__.__name__} failed to handle event "
...
class TaskEventPublisher:
    ...
    async def publish_task_failed(
        self, task_id: str, failure_data: Dict[str, Any]
    ) -> None:
    ...
    async def publish_milestone_reached(
        self, task_id: str, milestone_data: Dict[str, Any]
    ) -> None:
