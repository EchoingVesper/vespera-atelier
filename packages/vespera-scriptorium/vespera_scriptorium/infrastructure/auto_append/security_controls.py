...
from .condition_evaluator import Condition, LogicalExpression
...
class SecurityValidator:
    ...
    def _validate_conditions(
        self, expression: LogicalExpression, depth: int = 0
    ) -> List[str]:
    ...
    def _validate_single_condition(self, condition: Condition) -> List[str]:
        ...
        for pattern in self.compiled_patterns:
            if pattern.search(condition.field):
                issues.append(
                    f"Dangerous pattern in condition field: {condition.field}"
                )
        ...
        if isinstance(condition.value, str):
            ...
            for pattern in self.compiled_patterns:
                if pattern.search(condition.value):
                    issues.append(
                        f"Dangerous pattern in condition value: {condition.value[:100]}"
                    )
        ...
        for field_name, field_value in [
            ...
            if isinstance(field_value, str):
                for pattern in self.compiled_patterns:
                    if pattern.search(field_value):
                        issues.append(
                            f"Dangerous pattern in task {field_name}: {pattern.pattern}"
                        )
            ...
            if isinstance(value, str):
                for pattern in self.compiled_patterns:
                    if pattern.search(value):
                        issues.append(
                            f"Dangerous pattern in metadata {key}: {pattern.pattern}"
                        )
        ...
        for key, value in parameters.items():
            ...
            if isinstance(value, str):
                ...
                for pattern in self.compiled_patterns:
                    if pattern.search(value):
                        issues.append(
                            f"Dangerous pattern in parameter {key}: {pattern.pattern}"
                        )
    ...
    def _check_resource_exhaustion(self, rule: AutoAppendRule) -> List[str]:
        ...
        if rule.max_executions and rule.max_executions > 1000:
            issues.append(
                f"High execution limit may cause resource exhaustion: {rule.max_executions}"
            )
        ...
        if complexity > 100:
            issues.append(
                f"High condition complexity may impact performance: {complexity}"
            )

        return issues

    def _calculate_condition_complexity(
        self, expression: LogicalExpression, depth: int = 0
    ) -> int:
...
class ResourceMonitor:
    ...
    def set_limit(self, limit: ResourceLimit) -> None:
        ...
        logger.info(
            f"Set resource limit: {limit.resource_type.value} = {limit.max_value}"
        )
    ...
    def get_usage_statistics(
        self, resource_type: ResourceType, hours: int = 24
    ) -> Dict[str, Any]:
        ...
        recent_data = [
            record
            for record in self.usage_history[resource_type]
...
class SecurityManager:
    ...
    def validate_and_authorize_rule(self, rule: AutoAppendRule) -> Dict[str, Any]:
        ...
        critical_issues = [
            issue for issue in security_issues if "critical" in issue.lower()
        ]
    ...
    def get_security_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "resource_limits": {
                rt.value: limit.max_value for rt, limit in self.monitor.limits.items()
            },
            "statistics": {
                ...
                "critical_events_last_24h": len(
                    [
                        e
                        for e in self.monitor.security_events
