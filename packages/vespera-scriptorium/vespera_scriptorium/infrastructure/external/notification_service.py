...
class WebhookNotificationService(NotificationService):
    ...
    async def notify_task_completed(
        self, task_id: str, result: str, artifacts: list[str], metadata: Dict[str, Any]
    ) -> None:
    ...
    async def notify_task_failed(
        self, task_id: str, error: str, metadata: Dict[str, Any]
    ) -> None:
            ...
            async with aiohttp.ClientSession(
                ...
                async with session.post(
                    ...
                    if response.status >= 400:
                        ...
                        logger.debug(
                            f"Webhook notification sent successfully: {payload['event']}"
                        )
...
class LoggingNotificationService(NotificationService):
    ...
    async def notify_task_started(self, task_id: str, metadata: Dict[str, Any]) -> None:
        ...
        self.logger.log(
            self.log_level, f"NOTIFICATION: Task {task_id} started - {metadata}"
        )
    ...
    async def notify_task_failed(
        ...
        self.logger.log(
            self.log_level, f"NOTIFICATION: Task {task_id} failed - Error: {error}"
        )
