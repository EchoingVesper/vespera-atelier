...
async def handle_create_generic_task(args: Dict[str, Any]) -> List[types.TextContent]:
    ...
    if failed_post_validations:
        ...
        logger.warning(f"Post-creation validation issues: {error_messages}")
...
async def handle_update_task(args: Dict[str, Any]) -> List[types.TextContent]:
    ...
    return format_mcp_success_response(
        data=response_data, message=f"Task {task_id} updated successfully"
    )
...
async def handle_delete_task(args: Dict[str, Any]) -> List[types.TextContent]:
    ...
    logger.info(
        f"Deleting generic task: {task_id} (force={force}, archive={archive_instead})"
    )
...
async def handle_cancel_task(args: Dict[str, Any]) -> List[types.TextContent]:
    ...
    try:
        ...
        logger.info(
            f"Cancelling generic task: {task_id} (preserve_work={preserve_work})"
        )
...
async def handle_query_tasks(args: Dict[str, Any]) -> List[types.TextContent]:
    ...
    try:
                ...
                for dep in task_dict["dependencies"]:
                    if hasattr(dep, "dict"):
                        ...
                        for dep_field in ["dependency_type", "dependency_status"]:
                            if dep_dict.get(dep_field) and hasattr(
                                dep_dict[dep_field], "value"
                            ):
        ...
        response = {
            ...
            "query_summary": {
                "filters_applied": (
                    query_result.get("filters_applied", [])
                ...
                "pagination": {
                    ...
                    "page_count": (
                        query_result.get("page_count", 1)
                        if isinstance(query_result, dict)
                        else 1
                    ),
                    "has_more": (
                        query_result.get("has_more", False)
                        if isinstance(query_result, dict)
                        else False
                    ),
                },
    ...
    try:
        ...
        if isinstance(execution_context, dict):
            response = {
                ...
                "execution_instructions": execution_context.get(
                    "execution_instructions", []
                ),
                "dependencies_completed": execution_context.get(
                    "dependencies_completed", True
                ),
                "estimated_effort": execution_context.get(
...
async def handle_plan_task_legacy(args: Dict[str, Any]) -> List[types.TextContent]:
    ...
    logger.info(
        f"Planning task with legacy handler: {args.get('description', 'Unknown')[:50]}..."
    )
            ...
            subtasks.append(
                {
                    ...
                    "specialist": (
                        created_subtask.specialist_type.value
                        if created_subtask.specialist_type
                        else "generic"
                    ),
                }
            )
