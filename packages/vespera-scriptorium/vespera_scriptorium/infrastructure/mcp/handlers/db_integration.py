...
class ArtifactService:
    ...
    def __init__(self, base_dir: str = None):
        self.base_dir = (
            Path(base_dir or os.getcwd()) / ".vespera_scriptorium" / "artifacts"
        )
        self.base_dir.mkdir(parents=True, exist_ok=True)

    async def store_artifact(
        self, task_id: str, content: str, artifact_type: str, metadata: Dict[str, Any]
    ):
...
class RealTaskUseCase:
    ...
    def __init__(self):
        ...
        self.orchestrator = TaskOrchestrator(
            self.state_manager, self.specialist_manager
        )
    ...
    async def update_task(
        ...
        try:
            ...
            updated_task = Task(
                ...
                status=(
                    TaskStatus(update_data.get("status", existing_task.status.value))
                    if "status" in update_data
                    else existing_task.status
                ),
                complexity=(
                    ComplexityLevel(
                        update_data.get("complexity", existing_task.complexity.value)
                    )
                    if "complexity" in update_data
                    else existing_task.complexity
                ),
                specialist_type=update_data.get(
                    "specialist_type",
                    existing_task.context.get("specialist", "generic"),
                ),
        ...
        try:
            ...
            for task in all_tasks:
                ...
                if "created_after" in filters:
                    include_task &= task.created_at >= datetime.fromisoformat(
            ...
            logger.info(
                f"Query returned {len(task_dicts)} tasks out of {len(all_tasks)} total"
            )
...
def health_check() -> dict:
    ...
    try:
        ...
        result = {
            "status": "healthy",
            "message": "Real orchestrator integration working",
        }
