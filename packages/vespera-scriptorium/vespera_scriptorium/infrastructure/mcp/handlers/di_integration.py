...
from .compatibility.error_handlers import (
    ErrorHandlingMixin,
    TaskNotFoundError,
    TaskStateError,
)
...
class CleanArchTaskUseCase(ErrorHandlingMixin):
    ...
    async def update_task(
        ...
        try:
            ...
            if any(
                key in update_data
                for key in ["complexity", "specialist_type", "context"]
            ):
                ...
                if "specialist_type" in update_data:
                    existing_metadata["specialist_type"] = update_data[
                        "specialist_type"
                    ]
    ...
    async def delete_task(
        self, task_id: str, force: bool = False, archive_instead: bool = True
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            return self.formatter.format_delete_response(
                task_id,
                action_taken,
                {
    ...
    async def cancel_task(
        self, task_id: str, reason: str = "", preserve_work: bool = True
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if preserve_work:
                try:
                    ...
                    if hasattr(self.task_repository, "add_task_artifact"):
                        self.task_repository.add_task_artifact(
                            task_id, cancellation_artifact
                        )

                except Exception as artifact_error:
                    logger.warning(
                        f"Failed to preserve artifacts for task {task_id}: {artifact_error}"
                    )
            ...
            # Format response using compatibility layer
            return self.formatter.format_cancel_response(
                task_id,
                {
    ...
    async def _update_dependent_tasks_on_cancellation(self, cancelled_task_id: str):
        ...
        try:
            ...
            for task in dependent_tasks:
                ...
                logger.info(
                    f"Updated task {task['id']} due to cancelled dependency {cancelled_task_id}"
                )

        except Exception as e:
            logger.warning(
                f"Failed to update dependent tasks for cancelled task {cancelled_task_id}: {e}"
            )
        ...
        try:
            ...
            if specialist_type == "devops":
                ...
                specialist_prompts.append(
                    f"Apply {specialist_type} specialist knowledge to this task"
                )
