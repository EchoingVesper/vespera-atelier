...
def setup_logging():
    ...
    if not is_mcp_server:
        logger.info(
            f"Logging configured for CLI mode: level={log_level}, output=stdout"
        )
...
async def enable_dependency_injection():
    ...
    from ....reboot.reboot_integration import (
        get_reboot_manager,
    ...
    def configure_services(registrar):
        ...
        registrar.register_factory(
            StateManager, lambda container: StateManager()
        ).as_singleton()

        # Register RebootManager - singleton that gets initialized with StateManager
        registrar.register_factory(
            type(get_reboot_manager()), lambda container: get_reboot_manager()
        ).as_singleton()
        ...
        from ....infrastructure.database.sqlite.sqlite_task_repository import (
        ...
        def create_database_connection_manager(container):
            ...
            db_path = Path(".vespera_scriptorium") / "tasks.sqlite"
            db_url = f"sqlite:///{db_path.absolute()}"
            return DatabaseConnectionManager(db_url)

        registrar.register_factory(
            DatabaseConnectionManager, create_database_connection_manager
        ).as_singleton()
...
async def handle_initialize_session(args: Dict[str, Any]) -> List[types.TextContent]:
    ...
    try:
        ...
        try:
            from ....reboot.orchestrator import (
                get_hot_reload_orchestrator,
        ...
        try:
            ...
            from ....infrastructure.database.unified_manager import (
                get_database_manager,
                initialize_global_database_manager,
            )
            ...
            if db_manager is None:
                logger.info(
                    "Initializing multi-database architecture (SQLite + Vector + Graph)"
                )
            ...
            if db_manager.operational:
                ...
                try:
                    ...
                    session_state["database_warnings"] = [
                        f"Operational DB test failed: {str(db_test_error)}"
                    ]
        ...
        # Create Vespera Scriptorium directory if needed
        task_orchestrator_dir = working_path / ".vespera_scriptorium"
        ...
        response = {
            ...
            "working_directory": str(
                Path(args.get("working_directory", os.getcwd())).resolve()
            ),
...
async def handle_synthesize_results(args: Dict[str, Any]) -> List[types.TextContent]:
    ...
    try:
                    ...
                    text=json.dumps(
                        {
                            ...
                            "recovery_suggestions": [
                                "Provide a valid parent_task_id parameter"
                            ],
                        },
                        indent=2,
                    ),
        ...
        try:
            from ....infrastructure.mcp.handlers.db_integration import (
            ...
            parent_query = await use_case.query_tasks(
                {"task_id": parent_task_id, "limit": 1}
            )
            ...
            failed_query = await use_case.query_tasks(
                {"parent_task_id": parent_task_id, "status": ["failed"], "limit": 100}
            )
            ...
            synthesis_results = {
                ...
                "metrics": {
                    ...
                    "success_rate": round(
                        ...
                        / max(1, len(completed_subtasks) + len(failed_subtasks))
                        * 100,
                        1,
                    ),
                },
            ...
            for subtask in completed_subtasks:
                ...
                if task_dict.get("artifacts"):
                    artifacts = (
                        task_dict["artifacts"]
                        if isinstance(task_dict["artifacts"], list)
                        else [task_dict["artifacts"]]
                    )
                ...
                try:
                    ...
                    if isinstance(earliest_start, str):
                        start_dt = datetime.fromisoformat(
                            earliest_start.replace("Z", "+00:00")
                        )
                    else:
                    ...
                    if isinstance(latest_completion, str):
                        end_dt = datetime.fromisoformat(
                            latest_completion.replace("Z", "+00:00")
                        )
                    else:
                        end_dt = latest_completion

                    duration = end_dt - start_dt
                    synthesis_results["timeline"]["duration_minutes"] = round(
            ...
            response = {
                ...
                "parent_task_title": (
                    parent_task.title
                    if hasattr(parent_task, "title")
                    else parent_task.get("title", "Unknown")
                ),
            ...
            logger.info(
                f"Synthesized results for {parent_task_id}: {synthesis_results['metrics']['success_rate']}% success rate"
            )
        ...
        response = {
            ...
            "error": str(e),
            "error_type": type(e).__name__,
            "recovery_suggestions": ["Check input parameters", "Verify system health"],
        }

    return [
        types.TextContent(type="text", text=json.dumps(response, indent=2, default=str))
    ]


def _generate_synthesis_recommendations(
    synthesis_results: Dict[str, Any], failed_subtasks: list
) -> List[str]:
    ...
    if success_rate == 100:
        recommendations.append(
            "Excellent execution - consider using this approach as a template for future tasks"
        )
    elif success_rate >= 80:
        recommendations.append(
            "Strong performance - review failed subtasks for improvement opportunities"
        )
    elif success_rate >= 60:
        recommendations.append(
            "Moderate success - analyze failed subtasks to identify common issues"
        )
    else:
        recommendations.append(
            "Low success rate - recommend task restructuring and requirement clarification"
        )

    if failed_subtasks:
        recommendations.append(
            f"Address {len(failed_subtasks)} failed subtasks before marking parent task complete"
        )
    ...
    if synthesis_results.get("timeline", {}).get("duration_minutes"):
        ...
        if duration > 240:  # > 4 hours
            recommendations.append(
                "Consider breaking down similar tasks into smaller chunks for better manageability"
            )
        elif duration < 30:  # < 30 minutes
            recommendations.append(
                "Fast execution - this task structure works well for quick iterations"
            )

    if len(synthesis_results.get("artifacts_collected", [])) == 0:
        recommendations.append(
            "No artifacts collected - consider if deliverables were properly captured"
        )
...
async def handle_get_status(args: Dict[str, Any]) -> List[types.TextContent]:
    ...
    try:
        ...
        try:
            from ....infrastructure.mcp.handlers.di_integration import (
            ...
            failed_tasks = await use_case.query_tasks({"status": "failed", "limit": 50})
            ...
            if include_completed:
                ...
                response["task_summary"]["total_completed"] = len(
        ...
        if include_system_status:
            response["system_status"] = {
                ...
                "capabilities": {
                    "hot_reload": False,
                    "task_orchestration": True,
                    "database_persistence": response.get("database_status")
                    == "connected",
                },
            }
            ...
            try:
                ...
                if hot_reload:
                    ...
                    response["system_status"]["capabilities"]["hot_reload"] = (
                        reload_status.get("enabled", False)
                    )
            except Exception as e:
        ...
        if include_metrics:
            response["metrics"] = {
                "response_time_ms": round(
                    (asyncio.get_event_loop().time() - response["timestamp"]) * 1000, 2
                ),
        ...
        if task_id:
            ...
            response["message"] = f"Status retrieved for task {task_id}" + (
            ...
            total_tasks = (
                response["task_summary"]["total_active"]
                + response["task_summary"]["total_pending"]
                + response["task_summary"]["total_failed"]
            )
            if (
                include_completed
                and response["task_summary"]["total_completed"] is not None
            ):
                total_tasks += response["task_summary"]["total_completed"]
            response["message"] = (
                f"System status retrieved - {total_tasks} total tasks tracked"
            )
        ...
        if response["task_summary"]["total_active"] > 0:
            response["next_steps"].append(
                "Use orchestrator_execute_task to work on active tasks"
            )
        if response["task_summary"]["total_pending"] > 0:
            response["next_steps"].append(
                "Use orchestrator_execute_task to start pending tasks"
            )
        if response["task_summary"]["total_failed"] > 0:
            response["next_steps"].append(
                "Review failed tasks and use orchestrator_update_task to fix issues"
            )
        if not response["next_steps"]:
            response["next_steps"].append(
                "Use orchestrator_plan_task to create new tasks"
            )
...
async def handle_maintenance_coordinator(
    args: Dict[str, Any],
    ...
    try:
                    ...
                    text=json.dumps(
                        {
                            ...
                            "recovery_suggestions": [
                                "Provide a valid action parameter"
                            ],
                        },
                        indent=2,
                    ),
        ...
        if action == "scan_cleanup":
            ...
            await _perform_structure_validation(
                maintenance_results, validation_level, scope
            )
                    ...
                    text=json.dumps(
                        {
                            ...
                            "recovery_suggestions": [
                                "Use a valid action from the available list"
                            ],
                        },
                        indent=2,
                    ),
        ...
        logger.info(
            f"Maintenance action '{action}' completed: {operations_count} operations, {issues_count} issues"
        )
...
async def _perform_cleanup_scan(
    results: Dict[str, Any], scope: str, target_task_id: Optional[str]
):
    ...
    try:
        ...
        if scope == "specific_subtask" and target_task_id:
            ...
            task_query = await use_case.query_tasks(
                {"task_id": target_task_id, "limit": 1}
            )
            ...
            if task_query.get("tasks"):
                ...
                if task_dict.get("status") == "completed" and not task_dict.get(
                    "artifacts"
                ):
                    results["issues_found"].append(
                        f"Completed task {target_task_id} has no artifacts"
                    )

                if task_dict.get("status") == "failed" and not task_dict.get(
                    "error_details"
                ):
                    results["issues_found"].append(
                        f"Failed task {target_task_id} missing error details"
                    )
            ...
            for task in stale_tasks:
                ...
                if task_dict.get("started_at"):
                    results["issues_found"].append(
                        f"Task {task_dict.get('task_id')} has been in progress - may need attention"
                    )

            results["operations_performed"].append(
                f"scanned_{len(stale_tasks)}_active_tasks"
            )
...
async def _perform_structure_validation(
    results: Dict[str, Any], validation_level: str, scope: str
):
        ...
        if validation_level in ["comprehensive", "full_audit"]:
            ...
            for module_name in required_modules:
                try:
                    ...
                    results["operations_performed"].append(
                        f"validated_module_{module_name}"
                    )
                except ImportError as e:
                    results["issues_found"].append(
                        f"Missing or broken module: {module_name} - {str(e)}"
                    )
                    ...
                    try:
                        ...
                        if session_data.get("capabilities", {}).get("hot_reload"):
                            results["items_cleaned"].append(
                                f"Updated session documentation for {session_file.name}"
                            )

                    except Exception as e:
                        results["issues_found"].append(
                            f"Could not update session file {session_file.name}: {str(e)}"
                        )
            else:
                results["issues_found"].append(
                    "No session files found for documentation update"
                )
        else:
...
async def _perform_handover_preparation(results: Dict[str, Any], scope: str):
    ...
    try:
        ...
        active_query = await use_case.query_tasks(
            {"status": ["active", "in_progress"], "limit": 50}
        )
        ...
        results["items_cleaned"].append(
            f"Prepared handover summary: {handover_summary}"
        )
        ...
        try:
            ...
            system_health = {
                "hot_reload_enabled": (
                    hot_reload.get_status().get("enabled", False)
                    if hot_reload
                    else False
                ),
                "restart_available": restart is not None,
                "maintenance_mode": (
                    restart.get_status().get("maintenance_mode", False)
                    if restart
                    else False
                ),
            }
            ...
            results["items_cleaned"].append(
                f"System health check completed: {system_health}"
            )
...
def _generate_maintenance_next_steps(results: Dict[str, Any]) -> List[str]:
    ...
    if issues_count > 0:
        next_steps.append(
            f"Review and address {issues_count} issues identified during {action}"
        )

        # Add specific recommendations based on action type
        if action == "scan_cleanup":
            next_steps.append(
                "Consider running cleanup operations to resolve identified issues"
            )
        elif action == "validate_structure":
            next_steps.append(
                "Fix structural issues before proceeding with complex operations"
            )
        elif action == "update_documentation":
    ...
    if issues_count == 0 and cleaned_count == 0:
        ...
        if action == "prepare_handover":
            next_steps.append(
                "Handover preparation complete - system ready for transition"
            )
...
async def handle_list_sessions(args: Dict[str, Any]) -> List[types.TextContent]:
    ...
    try:
        ...
        # Get working directory and Vespera Scriptorium directory
        working_dir = Path.cwd()
        task_orchestrator_dir = working_dir / ".vespera_scriptorium"
                    ...
                    text=json.dumps(
                        {
                            "sessions": [],
                            "total_count": 0,
                            "message": "No .vespera_scriptorium directory found - no sessions available",
                        },
                        indent=2,
                    ),
                )
...
async def handle_resume_session(args: Dict[str, Any]) -> List[types.TextContent]:
    ...
    try:
        ...
        if not session_id:
            return [
                types.TextContent(
                    type="text",
                    text=json.dumps(
                        {"success": False, "error": "session_id is required"}, indent=2
                    ),
                )
            ]
        ...
        session_data["database_status"] = (
            "connected"  # Will be verified by database manager
        )
                    ...
                    text=json.dumps(
                        {
                            "cleaned_sessions": [],
                            "total_cleaned": 0,
                            "message": "No .vespera_scriptorium directory found",
                        },
                        indent=2,
                    ),
                )
                ...
                text=json.dumps(
                    {
                        ...
                        "dry_run": dry_run,
                        "older_than_days": (
                            older_than_days if cleanup_type == "old" else None
                        ),
                        ...
                        "next_steps": [
                            (
                                "Run with dry_run=false to actually perform cleanup"
                                if dry_run
...
async def handle_session_status(args: Dict[str, Any]) -> List[types.TextContent]:
    ...
    try:
        ...
        if (
            status["initialized"]
            and status["database_status"] == "connected"
            and db_connected
        ):
