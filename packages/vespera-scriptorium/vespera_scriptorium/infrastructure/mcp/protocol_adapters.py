...
from ...domain import OrchestrationError, TaskComplexity, TaskPriority
...
class MCPRequestAdapter:
    ...
    @staticmethod
    def validate_args(
        args: Dict[str, Any], required_fields: List[str]
    ) -> Optional[str]:
        """
        ...
        """
        missing = [
            field for field in required_fields if field not in args or not args[field]
        ]
        if missing:
...
class MCPResponseAdapter:
    ...
    def error_response(error: Exception) -> List[types.TextContent]:
        ...
        if isinstance(error, OrchestrationError):
            ...
            error_data = {"error": str(error), "type": error.__class__.__name__}

        return [types.TextContent(type="text", text=json.dumps(error_data, indent=2))]
    ...
    def progress_status_response(response) -> List[types.TextContent]:
        ...
        data = {"overall_status": response.overall_status, "metrics": response.metrics}
...
class MCPErrorAdapter:
    ...
    def validation_error(
        ...
        error_data = {
            ...
            or ["Check the input parameters", "Refer to tool documentation"],
        }
