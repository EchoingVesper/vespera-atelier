...
class JSON5Parser:
    ...
    def parse(self, content: str) -> Dict[str, Any]:
        ...
        if len(content) > self.max_size:
            raise JSON5ValidationError(
                f"Content exceeds maximum size of {self.max_size} bytes"
            )
    ...
    def _validate_structure(self, data: Any, depth: int = 0) -> None:
        ...
        if depth > self.max_depth:
            raise JSON5ValidationError(
                f"Structure exceeds maximum depth of {self.max_depth}"
            )
        ...
        if isinstance(data, dict):
            for key, value in data.items():
                if not isinstance(key, str):
                    raise JSON5ValidationError(
                        f"Object keys must be strings, got {type(key)}"
                    )
