...
class ContextManager:
    ...
    def __init__(self, base_workspace_dir: Optional[Path] = None):
        self.base_workspace_dir = (
            base_workspace_dir or Path.cwd() / ".vespera_scriptorium"
        )
    ...
    async def create_execution_context(
        ...
        await self.create_checkpoint(
            context, "initialization", "Initial execution context created"
        )
    ...
    async def update_execution_context(
        self, execution_id: str, hook_context: HookContext
    ) -> bool:
        """
        Update execution context from hook results.
    ...
    async def transition_phase(
        ...
        await self.create_checkpoint(
            context, checkpoint_name, f"Transitioned from {old_phase} to {new_phase}"
        )
    ...
    async def create_checkpoint(
        self, context: ExecutionContext, checkpoint_name: str, description: str = ""
    ) -> str:
        """
        ...
        """
        checkpoint_id = (
            f"{context.execution_id}_{checkpoint_name}_{datetime.now().timestamp()}"
        )
        ...
        logger.debug(
            f"Created checkpoint: {checkpoint_name} for {context.execution_id}"
        )
    ...
    async def recover_from_interruption(
        ...
        if checkpoint_name:
            ...
            checkpoint_data = (
                context.checkpoint_history[-1] if context.checkpoint_history else None
            )
        ...
        recovery_guidance = {
            ...
            "context_summary": checkpoint_data.get("recovery_hints", {}).get(
                "context_summary", ""
            ),
            "suggested_next_action": checkpoint_data.get("recovery_hints", {}).get(
                "next_suggested_action", ""
            ),
            "cognitive_state_notes": checkpoint_data.get("recovery_hints", {}).get(
                "cognitive_load_state", {}
            ),
            "recovery_success_probability": self._estimate_recovery_probability(
        ...
        context.interruption_points.append(
            {
                ...
                "interruption_duration_minutes": recovery_guidance[
        ...
        logger.info(
            f"Recovered context {execution_id} from interruption using checkpoint"
        )

        return context, recovery_guidance

    async def cleanup_context(
        self, execution_id: str, preserve_artifacts: bool = True
    ) -> bool:
        """
            ...
            contexts_info.append(
                {
                    ...
                    "current_phase": context.current_phase,
                    "progress_percentage": (
                        context.phase_index / max(context.total_phases, 1)
                    )
                    * 100,
                    "execution_status": context.execution_status,
    ...
    async def _persist_context(self, context: ExecutionContext) -> None:
        ...
        context_dict["active_agents"] = list(
            context_dict["active_agents"]
        )  # Convert set to list
        ...
        try:
            ...
            for timestamp_field in [
                ...
                if timestamp_field in context_dict and context_dict[timestamp_field]:
                    context_dict[timestamp_field] = datetime.fromisoformat(
                        context_dict[timestamp_field]
                    )
    ...
    async def _check_auto_checkpoint(self, context: ExecutionContext) -> None:
        ...
        last_checkpoint_time = datetime.fromisoformat(
            context.checkpoint_history[-1]["created_at"]
        )

        if datetime.now() - last_checkpoint_time > self.auto_checkpoint_interval:
            await self.create_checkpoint(
                context, "auto_checkpoint", "Automatic checkpoint for ED support"
            )
    ...
    def _estimate_recovery_probability(
        self, context: ExecutionContext, checkpoint_data: Dict[str, Any]
    ) -> float:
    ...
    async def _archive_context(
        self, context: ExecutionContext, preserve_artifacts: bool
    ) -> None:
        ...
        archive_data = {
            ...
            "total_duration_minutes": (
                context.last_activity_at - context.started_at
            ).total_seconds()
            / 60,
            "artifacts_count": len(context.execution_artifacts),
            "checkpoints_count": len(context.checkpoint_history),
...
class ContextBuilder:
    ...
    async def build_hook_context(
        ...
        if agent_id:
            ...
            if agent_id in execution_context.isolated_workspaces:
                hook_context.workspace_path = execution_context.isolated_workspaces[
                    agent_id
                ]
    ...
    async def _enrich_context_for_hook(
        ...
        hook_context.metadata["hook_execution_count"] = (
            execution_context.execution_metadata.get("hook_executions", {}).get(
                hook_id, 0
            )
        )

        # Add phase-specific information
        current_phase_config = execution_context.execution_metadata.get(
            "phase_configs", {}
        ).get(execution_context.current_phase, {})
        hook_context.metadata["current_phase_config"] = current_phase_config

        # Add cognitive load information
        hook_context.cognitive_load_indicators.update(
            execution_context.cognitive_load_tracking
        )
