...
from .base import ExecutiveDysfunctionHook, Hook, HookType
...
class HookRegistry:
    ...
    def find_hooks_by_features(
        self, required_features: List[str], hook_type: Optional[HookType] = None
    ) -> List[Hook]:
        """
        ...
        for hook_id, registration in self._hooks.items():
            ...
            if all(
                registration.ed_features.get(feature, False)
    ...
    def get_recommended_hooks_for_template(
        self, template_metadata: Dict[str, Any], hook_type: HookType
    ) -> List[Hook]:
        """
    ...
    def validate_hook_configuration(self, hook_ids: List[str]) -> Dict[str, List[str]]:
        """
    ...
    def _calculate_hook_score(
        self, hook: Hook, template_metadata: Dict[str, Any]
    ) -> float:
...
def get_hooks_for_type(
    hook_type: HookType, ed_only: bool = False, tags: Optional[List[str]] = None
) -> List[Hook]:
...
def find_hooks_by_features(
    required_features: List[str], hook_type: Optional[HookType] = None
) -> List[Hook]:
