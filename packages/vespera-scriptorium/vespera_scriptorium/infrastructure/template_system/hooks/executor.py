...
class ExecutionPlan:
    ...
    def add_group(self, hooks: List[Hook], is_parallel: bool = False) -> None:
        ...
        group_time = (
            max(hook.average_execution_time for hook in hooks)
            if is_parallel
            else sum(hook.average_execution_time for hook in hooks)
        )
...
class DependencyGraph:
    ...
    def resolve_execution_order(self) -> ExecutionPlan:
        ...
        while remaining_hooks:
            ...
            ready_hooks = [
                hook_id for hook_id in remaining_hooks if in_degree[hook_id] == 0
            ]
            ...
            is_parallel = len(ready_hooks) > 1 and self._can_execute_in_parallel(
                ready_hooks
            )
...
class HookExecutor:
    ...
    async def execute_hooks(
        self, hook_type: HookType, hooks: List[Hook], context: HookContext
    ) -> List[HookResult]:
        """
    ...
    def _resolve_execution_plan(
        self, graph: DependencyGraph, context: HookContext
    ) -> ExecutionPlan:
        ...
        # Log execution plan
        logger.info(
            f"Resolved execution plan: {optimized_plan.get_total_hooks()} hooks in {len(optimized_plan.execution_groups)} groups"
        )

        return optimized_plan

    def _optimize_for_ed(
        self, plan: ExecutionPlan, context: HookContext
    ) -> ExecutionPlan:
        ...
        for group_idx, group in enumerate(plan.execution_groups):
            ...
            if len(group) > self.overwhelm_threshold:
                logger.warning(
                    f"Hook group size ({len(group)}) exceeds overwhelm threshold ({self.overwhelm_threshold})"
                )
    ...
    async def _execute_plan(
        self, plan: ExecutionPlan, context: HookContext, hook_type: HookType
    ) -> List[HookResult]:
        ...
        try:
            for group_idx, group in enumerate(plan.execution_groups):
                ...
                logger.info(
                    f"Executing hook group {group_idx + 1}/{len(plan.execution_groups)}: {len(group)} hooks {'in parallel' if is_parallel else 'sequentially'}"
                )
    ...
    async def _execute_hooks_parallel(
        self, hooks: List[Hook], context: HookContext
    ) -> List[HookResult]:
    ...
    async def _execute_single_hook(
        ...
        try:
            ...
            logger.debug(
                f"Hook {hook.hook_id} completed: {result.success} ({execution_time:.1f}ms)"
            )
    ...
    def _update_context_with_results(
        self, context: HookContext, results: List[HookResult]
    ) -> None:
        ...
        for result in results:
            ...
            if result.checkpoint_created:
                context.checkpoint_data[result.checkpoint_created] = (
    ...
    async def _check_checkpoint_needed(self, context: HookContext) -> None:
        ...
        if (
            ...
            or now - self.last_checkpoint > self.checkpoint_interval
        ):
    ...
    async def _attempt_rollback(self, hooks: List[Hook], context: HookContext) -> None:
        ...
        rollback_hooks = [
            h for h in hooks if h.hook_id in executed_hook_ids and h.supports_rollback()
        ]

        if not rollback_hooks:
            logger.warning(
                "No hooks support rollback - execution state may be inconsistent"
            )
            return
    ...
    def _record_execution_success(
        ...
        logger.info(
            f"Execution {execution_id} completed successfully in {total_time:.1f}ms"
        )

    def _record_execution_failure(
        self, execution_id: str, hook_type: HookType, error: Exception
    ) -> None:
