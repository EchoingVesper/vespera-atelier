...
class TemplateEngine:
    ...
    def __init__(
        ...
        self.template_dir = (
            template_dir or Path.cwd() / ".vespera_scriptorium" / "templates"
        )
    ...
    def _resolve_template_inheritance(self, template_id: str) -> Dict[str, Any]:
        ...
        if template_id in self._inheritance_chain:
            raise TemplateValidationError(
                f"Circular inheritance detected: {template_id}"
            )
        ...
        for section in ["metadata", "parameters", "tasks", "variables"]:
            if section in child:
                if section in merged:
                    if isinstance(merged[section], dict) and isinstance(
                        child[section], dict
                    ):
                        merged[section].update(child[section])
                    elif isinstance(merged[section], list) and isinstance(
                        child[section], list
                    ):
    ...
    def _validate_parameters(
        self,
        param_definitions: List[TemplateParameter],
    ...
    def _validate_parameter_value(
        self, param_def: TemplateParameter, value: Any
    ) -> Any:
        ...
        if param_def.type == "string" and not isinstance(value, str):
            ...
            raise ParameterSubstitutionError(
                f"Parameter {param_def.name} must be an array"
            )
        elif param_def.type == "object" and not isinstance(value, dict):
            raise ParameterSubstitutionError(
                f"Parameter {param_def.name} must be an object"
            )
        ...
        if isinstance(value, str):
            if param_def.min_length is not None and len(value) < param_def.min_length:
                raise ParameterSubstitutionError(
                    f"Parameter {param_def.name} too short (min: {param_def.min_length})"
                )
            if param_def.max_length is not None and len(value) > param_def.max_length:
                raise ParameterSubstitutionError(
                    f"Parameter {param_def.name} too long (max: {param_def.max_length})"
                )
            if param_def.validation_pattern:
                if not re.match(param_def.validation_pattern, value):
                    raise ParameterSubstitutionError(
                        f"Parameter {param_def.name} doesn't match pattern"
                    )
        ...
        if isinstance(value, (int, float)):
            if param_def.min_value is not None and value < param_def.min_value:
                raise ParameterSubstitutionError(
                    f"Parameter {param_def.name} too small (min: {param_def.min_value})"
                )
            if param_def.max_value is not None and value > param_def.max_value:
                raise ParameterSubstitutionError(
                    f"Parameter {param_def.name} too large (max: {param_def.max_value})"
                )

        # Allowed values validation
        if (
            param_def.allowed_values is not None
        ...
        def check_placeholders(obj: Any, path: str = ""):
            if isinstance(obj, str):
                if "{{" in obj and "}}" in obj:
                    ...
                    if remaining:
                        raise ParameterSubstitutionError(
                            f"Unresolved parameters at {path}: {remaining}"
                        )
            elif isinstance(obj, dict):
