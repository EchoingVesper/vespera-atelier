...
class ConfigValidator:
    ...
    def _validate_workspace_config(self, config: Dict[str, Any]) -> List[str]:
        ...
        if config.get("workspace_dir"):
            ...
            if not workspace.exists():
                ...
                errors.append(
                    f"Workspace directory is not readable/writable: {workspace}"
                )

        # Check artifacts directory
        artifacts_dir = config.get("artifacts_dir", ".vespera_scriptorium/artifacts")
    ...
    def _validate_performance_config(self, config: Dict[str, Any]) -> List[str]:
        ...
        if not isinstance(max_tasks, int) or max_tasks <= 0:
            ...
            errors.append(
                "max_concurrent_tasks should not exceed 50 for performance reasons"
            )
