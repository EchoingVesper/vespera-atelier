...
class TaskErrorHandler(ErrorHandler):
    ...
    def handle(
        ...
        logger.warning(
            f"Task error in {context.get('task_id', 'unknown')}: {task_error}"
        )
...
class SpecialistErrorHandler(ErrorHandler):
    ...
    def handle(
        ...
        if "configuration" in str(specialist_error).lower():
            should_retry = False
            user_message = (
                "Specialist configuration error. Please check role definitions."
            )
            ...
            should_retry = False
            user_message = (
                "Selected specialist lacks required capabilities for this task."
            )
        ...
        return {
            ...
            "user_message": user_message,
            "details": (
                specialist_error.to_dict()
                if hasattr(specialist_error, "to_dict")
                else {
...
class InfrastructureErrorHandler(ErrorHandler):
    ...
    def handle(
        ...
        should_retry = (
            infra_error.is_recoverable
            if hasattr(infra_error, "is_recoverable")
            else True
        )
        ...
        return {
            ...
            "details": (
                infra_error.to_dict()
                if hasattr(infra_error, "to_dict")
                else {
...
class ErrorHandlerRegistry:
    ...
    def handle_error(
        ...
        sorted_handlers = sorted(
            self.handlers.items(), key=lambda x: x[0], reverse=True
        )
        ...
        logger.warning(
            f"No specific handler found for {type(error).__name__}, using default handler"
        )
    ...
    def get_registered_handlers(self) -> Dict[int, str]:
        ...
        return {
            priority: handler.__class__.__name__
            for priority, handler in self.handlers.items()
        }
