...
from ...domain.exceptions import RecoveryStrategy as RecoveryType
from ...domain.exceptions import (
    SpecialistError,
    TaskError,
)
...
class TaskRecoveryStrategy(RecoveryStrategy):
    ...
    def can_recover(
        ...
        if any(
            keyword in error_str for keyword in ["corruption", "validation", "security"]
        ):
            return False
    ...
    async def attempt_recovery(
        ...
        for strategy in self.strategies:
            if strategy.can_recover(error, context):
                logger.info(
                    f"Attempting recovery with {strategy.__class__.__name__} for {type(error).__name__}"
                )
                ...
                try:
                    ...
                    logger.error(
                        f"Recovery strategy {strategy.__class__.__name__} failed: {strategy_error}"
                    )
                    continue
