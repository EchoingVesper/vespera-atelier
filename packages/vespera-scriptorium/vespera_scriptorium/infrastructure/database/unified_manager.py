...
vector, and graph databases in the Vespera Scriptorium.
"""
...
class UnifiedDatabaseManager:
        ...
        try:
            ...
            if self.vector and embedding_data:
                ...
                vector_result = await self.vector.upsert_embeddings(
                    [embedding_data], namespace="tasks"
                )
    ...
    async def search_similar_tasks(
        self, query_text: str, top_k: int = 10, include_graph_context: bool = False
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            if self.vector:
                ...
                vector_results = await self.vector.search(
                    query_embedding=query_embedding, top_k=top_k, namespace="tasks"
                )
                ...
                for result in vector_results:
                    ...
                    if self.operational:
                        task_data = await self.operational.execute_one(
                            "SELECT * FROM tasks WHERE id = ?", {"id": task_id}
                        )
...
async def initialize_global_database_manager(
    ...
    if _global_database_manager is None:
        if config is None:
            ...
            # Get .vespera_scriptorium directory
        ...
        logger.info(
            "Global database manager initialized with multi-database architecture"
        )
