...
class ChromaDBAdapter(VectorDatabaseAdapter[Any]):
    ...
    using ChromaDB for RAG operations in the Vespera Scriptorium.
    """
        ...
        try:
            ...
            if self.persist_directory:
                self._client = chromadb.PersistentClient(
                    path=self.persist_directory, settings=self.client_settings
                )
            else:
    ...
    async def upsert_embeddings(
        self, embeddings: List[Dict[str, Any]], namespace: Optional[str] = None
    ) -> Dict[str, Any]:
        """
