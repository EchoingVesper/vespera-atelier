...
class SQLiteSpecialistRepository(SpecialistRepository):
    ...
    def _ensure_tables(self):
        ...
        with self.connection_manager.transaction() as conn:
            ...
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_usage_specialist ON specialist_usage(specialist_id)"
            )
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_usage_task ON specialist_usage(task_id)"
            )
    ...
    def get_specialist(self, specialist_id: str) -> Optional[Dict[str, Any]]:
        ...
        if row:
            ...
            specialist["configuration"] = (
                json.loads(specialist["configuration"])
    ...
    def get_specialist_template(self, template_name: str) -> Optional[Dict[str, Any]]:
        ...
        row = self.connection_manager.execute_one(
            "SELECT * FROM specialist_templates WHERE name = ?", (template_name,)
        )
    ...
    def record_specialist_usage(
        self, specialist_id: str, task_id: str, usage_data: Dict[str, Any]
    ) -> bool:
        ...
        with self.connection_manager.transaction() as conn:
            ...
            """,
                (usage_id, specialist_id, task_id, json.dumps(usage_data), now),
            )
    ...
    def get_specialist_metrics(
        ...
        if specialist_id:
            ...
            return {"specialists_usage": [dict(row) for row in usage_by_specialist]}

    def get_recommended_specialist(
        self, task_type: str, task_context: Dict[str, Any]
    ) -> Optional[str]:
