...
class AsyncSQLiteTaskRepository(AsyncTaskRepository):
    ...
    async def delete_task(self, task_id: str) -> bool:
        ...
        try:
            async with self.db_adapter.transaction() as tx:
                ...
                await self.db_adapter.execute(
                    "DELETE FROM task_artifacts WHERE task_id = ?", {"task_id": task_id}
                )

                # Delete the task
                await self.db_adapter.execute(
                    "DELETE FROM tasks WHERE id = ?", {"id": task_id}
                )
    ...
    async def get_task_hierarchy(self, root_task_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            results = await self.db_adapter.execute(
                query, {"root_task_id": root_task_id}
            )
            return results
    ...
    async def cleanup_old_tasks(
        ...
        try:
            ...
            if exclude_sessions:
                placeholders = ", ".join(
                    [f":session_{i}" for i in range(len(exclude_sessions))]
                )
            ...
            # Get tasks to delete
            old_tasks = await self.db_adapter.execute(
                f"""
    ...
    async def get_task_metrics(
        ...
        try:
            ...
            # Get basic counts
            counts_result = await self.db_adapter.execute(
                f"""
                SELECT 
                    status,
                    COUNT(*) as count
            ...
            # Get total count
            total_result = await self.db_adapter.execute_one(
                f"""
            ...
            # Get date range
            date_range_result = await self.db_adapter.execute_one(
                f"""
                SELECT 
            ...
            return {
                ...
                "date_range": {
                    "earliest": (
                        date_range_result["earliest"] if date_range_result else None
                    ),
                    "latest": (
                        date_range_result["latest"] if date_range_result else None
                    ),
                },
        ...
        try:
            ...
            if filters.get("search_query"):
                ...
                where_clauses.append(
                    "(title LIKE :search_query OR description LIKE :search_query)"
                )
            ...
            if filters.get("dependency_ids"):
                ...
                if dependency_ids:
                    placeholders = ", ".join(
                        [f":dep_{i}" for i in range(len(dependency_ids))]
                    )
                    where_clauses.append(
                        f"""
                        id IN (
            ...
            filters_applied = {
                ...
                if k not in ["page", "page_size", "sort_by", "sort_order"]
                and v is not None
            }
            ...
            return {
                ...
                "sorting": {"sort_by": sort_by, "sort_order": sort_order.lower()},
            }
