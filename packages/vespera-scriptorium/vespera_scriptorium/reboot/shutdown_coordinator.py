...
class ShutdownCoordinator:
    ...
    def __init__(
        ...
        self.status = ShutdownStatus(
            phase=ShutdownPhase.IDLE, progress_percent=0.0, message="Ready for shutdown"
        )
    ...
    async def emergency_shutdown(self) -> bool:
        ...
        try:
            self._update_status(
                ShutdownPhase.FINALIZING, 90.0, "Emergency shutdown in progress"
            )
            ...
            self._update_status(
                ShutdownPhase.COMPLETE, 100.0, "Emergency shutdown complete"
            )
            ...
            self._update_status(
                ShutdownPhase.FAILED, 0.0, f"Emergency shutdown failed: {e}"
            )
            return False
    ...
    async def _execute_shutdown_sequence(self, restart_reason: RestartReason):
        ...
        try:
            ...
            self._update_status(ShutdownPhase.FAILED, 0.0, f"Shutdown failed: {e}")
        finally:
            self._shutdown_in_progress = False
            self.shutdown_event.set()
    ...
    async def _enter_maintenance_mode(self):
        ...
        self._update_status(
            ShutdownPhase.MAINTENANCE_MODE, 10.0, "Entering maintenance mode"
        )
    ...
    async def _suspend_active_tasks(self):
        ...
        self._update_status(
            ShutdownPhase.SUSPENDING_TASKS, 25.0, "Suspending active tasks"
        )
            ...
            for callback in self.shutdown_callbacks:
                try:
                    await asyncio.wait_for(
                        callback(),
                        timeout=self.task_suspension_timeout
    ...
    async def _serialize_server_state(
        self, restart_reason: RestartReason
    ) -> ServerStateSnapshot:
        """Serialize complete server state."""
        self._update_status(
            ShutdownPhase.SERIALIZING_STATE, 50.0, "Serializing server state"
        )
        ...
        try:
            ...
            if state_manager is None:
                ...
                snapshot.integrity_hash = (
                    self.state_serializer._generate_integrity_hash(snapshot)
                )
            else:
                snapshot = await self.state_serializer.create_snapshot(
                    state_manager, restart_reason
                )
    ...
    async def _close_connections(self):
        ...
        self._update_status(
            ShutdownPhase.CLOSING_CONNECTIONS, 75.0, "Closing connections"
        )
    ...
    async def _final_cleanup(self):
        ...
        self._update_status(ShutdownPhase.FINALIZING, 90.0, "Performing final cleanup")
...
class ShutdownManager:
    ...
    @classmethod
    def get_instance(
        cls, state_serializer: Optional[StateSerializer] = None, **kwargs
    ) -> ShutdownCoordinator:
