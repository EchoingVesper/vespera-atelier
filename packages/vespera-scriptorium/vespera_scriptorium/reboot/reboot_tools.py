    ...
    try:
        ...
        if check_active_tasks:
            try:
                ...
                from ...infrastructure.database.sqlite.sqlite_repository_factory import (
                ...
                active_tasks = await task_repo.query_tasks(
                    status=[status.value for status in active_statuses], limit=100
                )

                shutdown_readiness["checks"]["active_tasks"] = {
                    "count": len(active_tasks),
                    "suspendable": len(active_tasks)
                    == 0,  # Only ready if no active tasks
                    "status": "ready" if len(active_tasks) == 0 else "has_active_tasks",
                    "active_task_ids": [
                        task.id for task in active_tasks[:5]
                    ],  # First 5 IDs
                }
        ...
        try:
            ...
            if session_id:
                ...
                test_results["results"]["session_test"] = {
                    ...
                    "status": (
                        "pass" if session_exists and server_responsive else "fail"
                    ),
                ...
                try:
                    ...
                    all_sessions = await session_repo.list_sessions(
            ...
            try:
                ...
                if history_file.exists():
                    ...
                    restart_status["history"] = {
                        "recent_restarts": history_data.get("restarts", [])[
                            -10:
                        ],  # Last 10
