...
Vespera Scriptorium server infrastructure.
"""
...
class RebootManager:
    ...
    async def initialize(self, state_manager: StateManager):
        ...
        self.shutdown_coordinator.add_shutdown_callback(
            self._prepare_state_manager_shutdown
        )
    ...
    async def trigger_restart(
        self, reason: RestartReason = RestartReason.MANUAL_REQUEST, timeout: int = 30
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            shutdown_complete = await self.shutdown_coordinator.wait_for_shutdown(
                timeout
            )
    ...
    async def get_restart_readiness(self) -> Dict[str, Any]:
        ...
        details = {
            ...
            "state_manager_initialized": (
                self.state_manager._initialized if self.state_manager else False
            ),
        }
        ...
        return {"ready": ready, "issues": issues, "details": details}
