...
class StateSerializer:
    ...
    def __init__(self, base_dir: str = None):
        ...
        Args:
            base_dir: Base directory for state files. Defaults to .vespera_scriptorium/server_state/
        """
        if base_dir is None:
            base_dir = os.path.join(os.getcwd(), ".vespera_scriptorium", "server_state")
        ...
        logger.info(
            f"StateSerializer initialized with state directory: {self.state_dir}"
        )
    ...
    async def create_snapshot(
        ...
        state_manager,
        restart_reason: RestartReason = RestartReason.MANUAL_REQUEST,
        include_client_sessions: bool = True,
    ) -> ServerStateSnapshot:
        ...
        async with self.lock:
            try:
                logger.info(
                    f"Creating state snapshot for restart reason: {restart_reason}"
                )
                ...
                snapshot.active_tasks = await self._serialize_active_tasks(
                    state_manager
                )
                snapshot.suspended_tasks = await self._serialize_suspended_tasks(
                    state_manager
                )

                # Capture database state
                snapshot.database_state = await self._serialize_database_state(
                    state_manager
                )
                ...
                logger.info(
                    f"State snapshot created successfully with {len(snapshot.active_tasks)} active tasks"
                )
                return snapshot
    ...
    async def save_snapshot(
        self, snapshot: ServerStateSnapshot, backup: bool = True
    ) -> str:
    ...
    async def cleanup_old_snapshots(self, keep_count: int = 5):
        ...
        try:
            ...
            logger.info(
                f"Cleanup completed, kept {min(len(backup_files), keep_count)} backups"
            )
    ...
    def _get_relevant_env_vars(self) -> Dict[str, str]:
        ...
        return {
            var: os.environ.get(var, "") for var in relevant_vars if os.environ.get(var)
        }
    ...
    def _snapshot_to_dict(self, snapshot: ServerStateSnapshot) -> Dict[str, Any]:
        ...
        if result["database_state"] and "last_checkpoint" in result["database_state"]:
            result["database_state"]["last_checkpoint"] = result["database_state"][
                "last_checkpoint"
            ].isoformat()
    ...
    def _dict_to_snapshot(self, data: Dict[str, Any]) -> ServerStateSnapshot:
        ...
        if data.get("database_state") and "last_checkpoint" in data["database_state"]:
            ...
            if isinstance(db_state["last_checkpoint"], str):
                db_state["last_checkpoint"] = datetime.fromisoformat(
                    db_state["last_checkpoint"]
                )
        ...
        for session_data in data.get("client_sessions", []):
            if "connected_at" in session_data and isinstance(
                session_data["connected_at"], str
            ):
                session_data["connected_at"] = datetime.fromisoformat(
                    session_data["connected_at"]
                )
            if "last_activity" in session_data and isinstance(
                session_data["last_activity"], str
            ):
