"""
Temporary File Staging System for Vespera Scriptorium
...
class StagingManager:
    ...
    async def create_staging(
        ...
        async with self._lock:
            ...
            logger.info(
                f"Created staging environment for request {request_id} at {staging_path}"
            )
            return context
        ...
        try:
            ...
            if chunk.checksum != expected_checksum:
                raise StagingIntegrityError(
                    f"Chunk checksum mismatch for {chunk.chunk_id}"
                )
            ...
            context.current_checksum = await self._calculate_file_checksum(
                context.partial_file
            )
            ...
            logger.debug(
                f"Appended chunk {chunk.chunk_id} to {request_id} "
                f"({chunk.size_bytes()} bytes, total: {context.bytes_written})"
            )

        except Exception as e:
            logger.error(
                f"Failed to append chunk {chunk.chunk_id} to {request_id}: {str(e)}"
            )
    ...
    async def finalize_staging(self, request_id: str) -> Path:
        ...
        try:
            ...
            async with aiofiles.open(
                context.partial_file, "r", encoding="utf-8"
            ) as src:
    ...
    async def atomic_move_to_production(
        self, staging_file: Path, production_path: Path
    ) -> None:
        """
        ...
        try:
            ...
            if staging_stat.st_dev == production_stat.st_dev:
                ...
                logger.info(
                    f"Atomic move (same filesystem): {staging_file} -> {production_path}"
                )
            else:
                # Different filesystem - copy, verify, then delete
                await self._cross_filesystem_atomic_move(staging_file, production_path)
                logger.info(
                    f"Atomic move (cross filesystem): {staging_file} -> {production_path}"
                )

        except Exception as e:
            logger.error(
                f"Atomic move failed: {staging_file} -> {production_path}: {str(e)}"
            )
    ...
    async def cleanup_staging(self, request_id: str) -> None:
        ...
        try:
            ...
            if context and context.staging_path.exists():
                ...
                logger.info(
                    f"Cleaned up staging for {request_id}: {context.staging_path}"
                )
            ...
            for staging_dir in self.staging_dir.iterdir():
                ...
                if metadata_file.exists():
                    try:
                        ...
                        logger.warning(
                            f"Error checking staging dir {staging_dir}: {str(e)}"
                        )
                else:
            ...
            if staging_dir.is_dir() and staging_dir.name.startswith(request_id):
                ...
                if metadata_file.exists():
                    try:
                        ...
                        logger.warning(
                            f"Error reading metadata for {request_id}: {str(e)}"
                        )
    ...
    @asynccontextmanager
    async def staging_operation(
        self,
        request_id: str,
        operation_type: StagingOperation = StagingOperation.BATCH,
        estimated_size: int = 0,
        auto_cleanup: bool = True,
    ) -> AsyncContextManager[StagingContext]:
        """
    ...
    async def _cross_filesystem_atomic_move(self, source: Path, dest: Path) -> None:
        ...
        try:
            ...
            raise AtomicMoveError(
                f"Cross-filesystem atomic move failed: {str(e)}"
            ) from e


# Factory function for easy integration
def create_staging_manager(
    base_dir: Union[str, Path], cleanup_interval_hours: int = 24
) -> StagingManager:
    """
    ...
    staging_dir = base_path / ".vespera_scriptorium"
