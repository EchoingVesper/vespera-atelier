#!/usr/bin/env python3
"""
Vespera Scriptorium Server - Clean Architecture Entry Point
...
async def main():
    ...
    try:
        ...
        enable_auto_reload = os.environ.get("MCP_AUTO_RELOAD", "false").lower() in (
            "true",
            "1",
            "yes",
        )
        if enable_auto_reload:
            try:
                from .monitoring.auto_reload import (
                    enable_auto_reload as start_auto_reload,
                )
        ...
        logger.info("Vespera Scriptorium server stopped")
