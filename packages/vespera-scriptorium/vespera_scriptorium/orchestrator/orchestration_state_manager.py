...
# Configure logging
logger = logging.getLogger("vespera_scriptorium.state")
...
class StateManager:
        ...
        if db_path is None:
            try:
                ...
                if not db_path:
                    ...
                    db_path = (
                        os.getcwd() + "/.vespera_scriptorium/vespera_scriptorium.db"
                    )
    ...
    async def _recover_interrupted_tasks(self):
        ...
        try:
            ...
            logger.info(
                f"Found {len(active_task_ids)} active tasks in persistent storage"
            )
    ...
    async def store_task_breakdown(self, breakdown: Task):
        ...
        async with self.lock:
            ...
            try:
                ...
                logger.info(
                    f"Saved task breakdown {breakdown.parent_task_id} to persistent storage"
                )
            except Exception as e:
                logger.error(
                    f"Failed to save task breakdown to persistent storage: {str(e)}"
                )
                raise
                ...
                if breakdown:
                    for subtask in breakdown.subtasks:
                        if subtask.task_id == task_id:
                            logger.info(
                                f"Retrieved subtask {task_id} from persistent storage"
                            )
                            return subtask
    ...
    async def update_subtask(self, subtask: Task):
        ...
        async with self.lock:
            ...
            try:
                ...
                parent_task_id = await self.persistence.get_parent_task_id(
                    subtask.task_id
                )
                ...
                if parent_task_id:
                    ...
                    logger.info(
                        f"Updated subtask {subtask.task_id} in persistent storage"
                    )
                    ...
                    logger.warning(
                        f"Could not find parent task ID for subtask {subtask.task_id}"
                    )
    ...
    async def _check_and_archive_parent_task(self, parent_task_id: str) -> None:
        ...
        if subtasks and all(st.status == TaskStatus.COMPLETED for st in subtasks):
            try:
                ...
                logger.error(
                    f"Failed to archive completed task {parent_task_id}: {str(e)}"
                )
                # Continue execution even if archiving fails

    async def _get_subtasks_for_parent_unlocked(
        ...
        try:
            ...
            if breakdown:
                logger.info(
                    f"Retrieved subtasks for parent task {parent_task_id} from persistent storage"
                )
            ...
            logger.error(
                f"Failed to retrieve subtasks for parent task {parent_task_id}: {str(e)}"
            )
            return []
        ...
        async with self.lock:
            ...
            try:
                ...
                for parent_task_id in active_task_ids:
                    try:
                        breakdown = await self.persistence.load_task_breakdown(
                            parent_task_id
                        )
                        if breakdown:
                        ...
                        logger.error(
                            f"Failed to load task {parent_task_id} from persistent storage: {str(e)}"
                        )
            except Exception as e:
