...
def find_role_files(project_dir: Optional[str] = None) -> List[Path]:
    ...
    # First, check in .vespera_scriptorium/roles/ directory (highest priority)
    task_orchestrator_roles_dir = project_path / ".vespera_scriptorium" / "roles"
    ...
    default_role_file = (
        Path(__file__).parent.parent.parent / "config" / "default_roles.yaml"
    )
...
def create_project_roles_file(project_dir: Optional[str] = None) -> Tuple[bool, Path]:
    ...
    project_path = Path(project_dir)
    # Create .vespera_scriptorium/roles directory
    roles_dir = project_path / ".vespera_scriptorium" / "roles"
...
#
# Note: This file overrides the default roles in the Vespera Scriptorium
...
def get_roles(project_dir: Optional[str] = None) -> Dict:
    ...
    if not role_files or all(f.name == "default_roles.yaml" for f in role_files):
        ...
        project_path = Path(project_dir)
        task_orchestrator_dir = project_path / ".vespera_scriptorium" / "roles"

        # Check if there are any files in the Vespera Scriptorium roles directory
        ...
        project_role_files = [
            f for f in project_path.glob("*.yaml") if f.stem.endswith("_roles")
        ]
