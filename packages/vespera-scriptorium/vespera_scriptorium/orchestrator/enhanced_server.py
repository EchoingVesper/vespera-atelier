...
class EnhancedArtifactServer:
    ...
    async def enhanced_complete_subtask(self, args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                logger.error(
                    f"Both enhanced and legacy methods failed for task {task_id}: {str(fallback_error)}"
                )
                return {
                    "task_id": task_id,
                    "status": "error",
    ...
    async def _analyze_response_quality(
        ...
        logger.info(
            f"Response analysis for task {task_id}: confidence={analysis['confidence_score']:.2f}, complete={analysis['is_likely_complete']}"
        )
        return analysis
    ...
    async def _store_complete_response(
        self,
        task_id: str,
        summary: str,
        detailed_work: str,
        ...
        return {
            ...
            "artifact_info": {
                ...
                "accessible_via": f".vespera_scriptorium/artifacts/{task_id}/{session.artifact_id}.md",
            },
            "context_saving": {
        ...
        async with session.write_stream():
            ...
            truncation_notice.extend(
                [
                    ...
                    f"- Partial file: `.vespera_scriptorium/temp/{task_id}/{session.artifact_id}/{session.artifact_id}_partial.md`",
                    "",
    ...
    async def resume_partial_artifact(
        ...
        """
        session = await self.streaming_manager.resume_partial_session(
            task_id, artifact_id
        )
    ...
    async def continue_partial_artifact(
        self,
        task_id: str,
        artifact_id: str,
        ...
        try:
            ...
            logger.error(
                f"Error continuing partial artifact {task_id}/{artifact_id}: {str(e)}"
            )
    ...
    async def cleanup_old_partials(self, max_age_hours: int = 24) -> Dict[str, Any]:
        ...
        Returns:
            Cleanup results
        """
        cleaned_count = await self.streaming_manager.cleanup_old_temp_files(
            max_age_hours
        )
...
async def example_streaming_usage():
    ...
    if result["status"] == "partial_response_detected":
        ...
        if continuation_result["success"]:
            print(
                f"Artifact completed successfully: {continuation_result['final_info']}"
            )
