...
class DecisionCapture:
    ...
    async def capture_simple_decision(
        self,
        title: str,
        decision: str,
        rationale: str,
    ...
    async def _get_next_decision_number(self) -> int:
        ...
        max_number = (
            self.db_session.query(ArchitecturalDecisionModel.decision_number)
            .filter_by(session_id=self.session_id)
            .order_by(ArchitecturalDecisionModel.decision_number.desc())
            .first()
        )
...
class DecisionManager:
    ...
    def create_decision_capture(
        self, subtask_id: str, session_id: str, specialist_type: str
    ) -> DecisionCapture:
    ...
    async def get_decisions_for_subtask(
        self, subtask_id: str
    ) -> List[ArchitecturalDecisionModel]:
        """Get all decisions made during a specific subtask."""
        return (
            self.db_session.query(ArchitecturalDecisionModel)
            .filter_by(subtask_id=subtask_id)
            .order_by(ArchitecturalDecisionModel.decision_number)
            .all()
        )

    async def get_decisions_for_session(
        self, session_id: str
    ) -> List[ArchitecturalDecisionModel]:
    ...
    async def generate_decision_context_summary(
        self, subtask_id: str
    ) -> Dict[str, Any]:
        """
        ...
        for decision in decisions:
            ...
            summary["decisions_by_category"][category] = (
                summary["decisions_by_category"].get(category, 0) + 1
            )

            # Count by impact
            impact = decision.impact_level
            summary["decisions_by_impact"][impact] = (
                summary["decisions_by_impact"].get(impact, 0) + 1
            )
        ...
        if decisions:
            summary["context_for_recovery"] = self._generate_context_narrative(
                decisions, summary
            )
    ...
    def _generate_context_narrative(
        self, decisions: List[ArchitecturalDecisionModel], summary: Dict[str, Any]
    ) -> str:
        """Generate a human-readable context narrative from decisions."""
        narrative_parts = []

        narrative_parts.append(
            f"During this subtask, {len(decisions)} architectural decisions were made."
        )
        ...
        if summary["affected_files"]:
            ...
            if len(summary["affected_files"]) > 5:
                narrative_parts.append(
                    f" and {len(summary['affected_files']) - 5} others"
                )
        ...
        if summary["outstanding_risks"]:
            ...
            for risk in summary["outstanding_risks"][:3]:  # Show top 3 risks
                narrative_parts.append(
                    f"- {risk['risk']} (from {risk['decision_title']})"
                )

        return "".join(narrative_parts)

    async def create_decision_evolution(
        self,
        ...
        if evolution_type == EvolutionType.SUPERSEDED:
            original_decision = (
                self.db_session.query(ArchitecturalDecisionModel)
                .filter_by(decision_id=original_decision_id)
                .first()
            )
    ...
    async def search_decisions(
        ...
        if impact_level:
            query = query.filter(
                ArchitecturalDecisionModel.impact_level == impact_level.value
            )
...
def create_decision_capture_for_subtask(
    subtask_id: str, session_id: str, specialist_type: str, db_session: Session
) -> DecisionCapture:
    """
...
async def generate_context_recovery_summary(
    subtask_id: str, db_session: Session
) -> Dict[str, Any]:
    """
