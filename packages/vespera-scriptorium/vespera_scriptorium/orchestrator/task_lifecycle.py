"""
Task Lifecycle Management for the Vespera Scriptorium.
...
from ..db.models import (
    MaintenanceOperationModel,
    StaleTaskTrackingModel,
    SubTaskModel,
    TaskArchiveModel,
    TaskBreakdownModel,
    TaskLifecycleModel,
...
from .models import SpecialistType, TaskStatus
...
class TaskLifecycleManager:
    ...
    def __init__(
        self,
        state_manager: DatabasePersistenceManager,
    ...
    async def detect_stale_tasks(
        ...
        """
        self.logger.info(
            f"Starting stale task detection (comprehensive={comprehensive_scan})"
        )
        ...
        try:
            with self.state_manager.session_scope() as session:
                ...
                active_tasks = (
                    ...
                    .filter(SubTaskModel.status.in_(["active", "pending"]))
                    .all()
                )
                ...
                for task in active_tasks:
                    ...
                    specialist_threshold = self.stale_detection_config[
                        "specialist_thresholds"
                    ].get(
                        ...
                        stale_reasons.append(
                            {
                                ...
                                "severity": (
                                    "high"
                                    if task_age_hours > specialist_threshold * 2
                                    else "medium"
                                ),
                            }
                        )
                    ...
                    parent_exists = (
                        session.query(TaskBreakdownModel)
                        .filter_by(parent_task_id=task.parent_task_id)
                        .first()
                    )
                    ...
                    if comprehensive_scan:
                        ...
                        workflow_stale_reasons = await self._check_workflow_abandonment(
                            session, task
                        )
                        stale_reasons.extend(workflow_stale_reasons)

                        # Check for dependency failures
                        dependency_stale_reasons = (
                            await self._check_dependency_failures(session, task)
                        )
                    ...
                    if stale_reasons:
                        stale_task_info = {
                            ...
                            "auto_cleanup_eligible": self._is_auto_cleanup_eligible(
                                stale_reasons
                            ),
                        }
    ...
    async def _check_workflow_abandonment(
        self, session, task: SubTaskModel
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            active_tasks = sum(
                1 for t in workflow_tasks if t.status in ["active", "pending"]
            )
            ...
            if (
                workflow_age_hours
                > self.stale_detection_config["workflow_abandonment_threshold_hours"]
                and completion_rate
                < self.stale_detection_config["minimum_progress_threshold"]
            ):
            ...
            if completion_rate > 0.1 and active_tasks > 0:
                ...
                if (
                    hours_since_activity
                    > self.stale_detection_config["default_threshold_hours"] * 2
                ):
                    reasons.append(
                        {
                            "reason": StaleTaskReason.ABANDONED_WORKFLOW.value,
            ...
            self.logger.error(
                f"Error checking workflow abandonment for task {task.task_id}: {str(e)}"
            )
            return []

    async def _check_dependency_failures(
        self, session, task: SubTaskModel
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for dep_id in task.dependencies:
                ...
                if not dep_task:
                    ...
                    dep_age_hours = (
                        datetime.utcnow() - dep_task.created_at
                    ).total_seconds() / 3600
                    dep_threshold = self.stale_detection_config[
                        "specialist_thresholds"
                    ].get(
                    ...
                    if (
                        dep_age_hours > dep_threshold * 1.5
                    ):  # Dependencies get extra time
                        reasons.append(
                            {
                                "reason": StaleTaskReason.DEPENDENCY_FAILURE.value,
            ...
            self.logger.error(
                f"Error checking dependency failures for task {task.task_id}: {str(e)}"
            )
            return []
    ...
    def _is_auto_cleanup_eligible(self, stale_reasons: List[Dict[str, Any]]) -> bool:
        ...
        if (
            StaleTaskReason.DEPENDENCY_FAILURE.value in reason_types
            and "critical" in severities
        ):
            return False
    ...
    async def archive_task(
        self, task_info: Dict[str, Any], archive_reason: str
    ) -> Dict[str, Any]:
            ...
            with self.state_manager.session_scope() as session:
                ...
                if task.artifacts:
                    try:
                        ...
                        archive_artifact = await self._create_archive_artifact(
                            task, archive_reason
                        )
                        artifacts_preserved = True
                        artifact_references = task.artifacts + [
                            archive_artifact["accessible_via"]
                        ]
                    except Exception as e:
                        self.logger.warning(
                            f"Failed to preserve artifacts for task {task_id}: {str(e)}"
                        )
                ...
                archive_data = {
                    ...
                    "archive_metadata": {
                        ...
                        "recommended_action": task_info.get(
    ...
    async def _create_archive_artifact(
        self, task: SubTaskModel, archive_reason: str
    ) -> Dict[str, Any]:
...
---
*This archive was created automatically by the Vespera Scriptorium maintenance system.*
"""
    ...
    async def cleanup_expired_archives(self) -> Dict[str, Any]:
        ...
        try:
            with self.state_manager.session_scope() as session:
                ...
                expired_archives = (
                    session.query(TaskArchiveModel)
                    .filter(TaskArchiveModel.expires_at <= current_time)
                    .all()
                )
                ...
                for archive in expired_archives:
                    try:
                        ...
                        self.logger.error(
                            f"Error processing archive {archive.id}: {str(e)}"
                        )
                        cleanup_results.append(
                            {
                                "archive_id": archive.id,
                ...
                deleted_count = sum(
                    1 for r in cleanup_results if r["action"] == "deleted"
                )
                extended_count = sum(
                    1 for r in cleanup_results if r["action"] == "retention_extended"
                )

                self.logger.info(
                    f"Archive cleanup completed: {deleted_count} deleted, {extended_count} extended"
                )
    ...
    async def perform_workspace_cleanup(
        ...
        try:
                ...
                try:
                    if stale_task["auto_cleanup_eligible"] or aggressive:
                        ...
                        if action in ["archive_stale_task", "archive_workflow"]:
                            ...
                            removal_result = (
                                await self._remove_orphaned_task_completely(stale_task)
                            )
                    ...
                    self.logger.error(
                        f"Error processing stale task {stale_task['task_id']}: {str(e)}"
                    )
            ...
            for workflow in incomplete_workflows:
                try:
                    if workflow["should_cleanup"] or aggressive:
                        workflow_result = await self._resolve_incomplete_workflow(
                            workflow, aggressive
                        )
                    ...
                    self.logger.error(
                        f"Error resolving workflow {workflow['parent_task_id']}: {str(e)}"
                    )
                    cleanup_results["errors"].append(
                        {
            ...
            cleanup_results["archives_cleaned"] = archive_cleanup.get(
                "archives_cleaned", 0
            )
            ...
            self.logger.info(
                f"Workspace cleanup completed: {cleanup_results['total_actions']} total actions"
            )
    ...
    async def _remove_orphaned_task_completely(
        ...
        try:
            with self.state_manager.session_scope() as session:
                ...
                archive_record = TaskArchiveModel(
                    ...
                    + timedelta(days=7),  # Short retention for orphaned tasks
                )
        ...
        try:
            with self.state_manager.session_scope() as session:
                ...
                for parent_task in parent_tasks:
                    ...
                    subtasks = (
                        session.query(SubTaskModel)
                        .filter_by(parent_task_id=parent_task.parent_task_id)
                        .all()
                    )
                    ...
                    completed_subtasks = sum(
                        1 for t in subtasks if t.status == "completed"
                    )
                    failed_subtasks = sum(1 for t in subtasks if t.status == "failed")
                    active_subtasks = sum(
                        1 for t in subtasks if t.status in ["active", "pending"]
                    )
                    ...
                    if (
                        oldest_task_age > 168 and completion_rate < 0.1
                    ):  # 1 week old, <10% complete
                        should_cleanup = True
    ...
    async def _resolve_incomplete_workflow(
        self, workflow_info: Dict[str, Any], aggressive: bool
    ) -> Dict[str, Any]:
        ...
        try:
            with self.state_manager.session_scope() as session:
                ...
                if completion_rate > 0.7 and aggressive:
                    ...
                    result = await self._complete_remaining_workflow_tasks(
                        subtasks, session
                    )
                    ...
                    result = await self._archive_entire_workflow(
                        parent_task_id, subtasks, "high_failure_rate"
                    )
                    ...
                    result = await self._archive_entire_workflow(
                        parent_task_id, subtasks, "stalled"
                    )
            ...
            self.logger.error(
                f"Error resolving incomplete workflow {parent_task_id}: {str(e)}"
            )
            return {
                "status": "failed",
                "parent_task_id": parent_task_id,
    ...
    async def _archive_entire_workflow(
        self, parent_task_id: str, subtasks: List[SubTaskModel], reason: str
    ) -> Dict[str, Any]:
        ...
        with self.state_manager.session_scope() as session:
            ...
            for subtask in subtasks:
                ...
                archived_tasks.append(
                    {"task_id": subtask.task_id, "status": archive_result["status"]}
                )
            ...
            if parent_task:
                ...
                workflow_archive = TaskArchiveModel(
                    ...
                    artifacts_preserved=False,
                    expires_at=datetime.utcnow()
                    + timedelta(days=self.archive_retention_config.get(reason, 30)),
                )
    ...
    async def _optimize_database_performance(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            optimizations.append(
                {"action": "database_optimization", "status": "failed", "error": str(e)}
            )

        return optimizations

    async def _update_stale_task_tracking(
        self, stale_tasks: List[Dict[str, Any]]
    ) -> None:
        ...
        try:
            with self.state_manager.session_scope() as session:
                for stale_task in stale_tasks:
                    ...
                    existing_record = (
                        session.query(StaleTaskTrackingModel)
                        .filter_by(task_id=stale_task["task_id"])
                        .first()
                    )

                    if existing_record:
                        # Update existing record
                        existing_record.stale_indicators = [
                            r["reason"] for r in stale_task["stale_reasons"]
                        ]
                        existing_record.auto_cleanup_eligible = stale_task[
                        ...
                        tracking_record = StaleTaskTrackingModel(
                            ...
                            - timedelta(hours=stale_task["age_hours"]),
                            stale_indicators=[
                                r["reason"] for r in stale_task["stale_reasons"]
                            ],
    ...
    async def get_lifecycle_statistics(self) -> Dict[str, Any]:
        ...
        try:
            with self.state_manager.session_scope() as session:
                ...
                archive_reasons = (
                    session.query(TaskArchiveModel.archive_reason).distinct().all()
                )
                for (reason,) in archive_reasons:
                    count = (
                        session.query(TaskArchiveModel)
                        .filter_by(archive_reason=reason)
                        .count()
                    )
                ...
                recent_transitions = (
                    session.query(TaskLifecycleModel)
                    .filter(
                        TaskLifecycleModel.created_at
                ...
                auto_cleanup_eligible = (
                    session.query(StaleTaskTrackingModel)
                    .filter_by(auto_cleanup_eligible=True)
                    .count()
                )
