"""
Smart Working Directory Detection System for Vespera Scriptorium.
...
class DirectoryDetector:
    ...
    PROJECT_MARKERS = {
        ...
        "Cargo.lock": {
            "confidence": 7,
            "type": "rust",
            "description": "Rust dependencies lock",
        },
        # Go projects
        ...
        "go.sum": {
            "confidence": 7,
            "type": "go",
            "description": "Go dependencies checksum",
        },
        # Java projects
    ...
    def find_project_markers(
        self, path: Path, max_depth: int = 3
    ) -> List[ProjectMarker]:
        """
    ...
    def validate_directory(self, path: str) -> DirectoryValidation:
        ...
        try:
            ...
            is_valid = (
                exists
                and path_obj.is_dir()
                and (is_writable or not self.security_checks)
            )
    ...
    def _try_git_root_detection(
        self, starting_path: Optional[str]
    ...
    def _try_project_marker_detection(
        self, starting_path: Optional[str]
    ) -> Optional[DetectionResult]:
        ...
        try:
            if get_config is not None:
                ...
                if (
                    ...
                    and Path(config.paths.workspace_dir).exists()
                ):
    ...
    def _apply_fallback_hierarchy(
        self, starting_path: Optional[str]
    ) -> DetectionResult:
        """Apply fallback hierarchy when primary methods fail."""
        fallback_paths = [
            (
                Path(starting_path or os.getcwd()).resolve(),
        ...
        return DetectionResult(
            ...
            confidence=1,
            validation=DirectoryValidation(
                is_valid=True, is_writable=True, exists=True, is_secure=True
            ),
            project_markers=[],
            fallback_used=True,
        )
