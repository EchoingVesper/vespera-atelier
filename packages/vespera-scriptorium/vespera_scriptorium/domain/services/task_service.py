...
class TaskService:
    ...
    def create_task(
        ...
        self.state_repository.record_event(
            session_id,
            "task_created",
            {"task_id": task_id, "title": title, "parent_task_id": parent_task_id},
        )
    ...
    def complete_task(
        ...
        if task["status"] in ("completed", "failed", "cancelled"):
            raise ValueError(
                f"Task {task_id} already completed with status: {task['status']}"
            )
        ...
        if success and result:
            ...
            self.task_repository.add_task_artifact(
                task_id, {"type": "result", "name": "task_result", "content": result}
            )
    ...
    def get_task_hierarchy(
        self, task_id: str, include_completed: bool = True
    ) -> Dict[str, Any]:
        """
