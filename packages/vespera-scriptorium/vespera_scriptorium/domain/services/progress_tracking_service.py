...
class ProgressTrackingService:
    ...
    async def get_status(
        ...
        for root_task in root_tasks:
            task_status = await self._build_task_status(
                root_task, tasks_by_parent, include_completed
            )
    ...
    async def complete_task(
        ...
        # Add result artifact
        self.task_repo.add_task_artifact(
            task_id, {"type": "result", "name": "completion_result", "content": results}
        )
    ...
    async def _build_task_status(
        ...
        for subtask in subtasks:
            subtask_status = await self._build_task_status(
                subtask, tasks_by_parent, include_completed
            )
    ...
    async def _check_parent_task_progress(
        ...
        completed_count = sum(
            1 for s in siblings if s["status"] in ("completed", "failed", "cancelled")
        )
        ...
        parent_ready = all_completed and parent_task["status"] not in (
            "completed",
            "failed",
            "cancelled",
        )
    ...
    async def _get_next_recommended_task(
        ...
        if parent_id:
            siblings = [
                t for t in actionable_tasks if t.get("parent_task_id") == parent_id
            ]
            if siblings:
