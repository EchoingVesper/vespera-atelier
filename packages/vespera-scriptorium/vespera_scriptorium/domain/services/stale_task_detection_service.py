...
class StaleTaskDetectionService:
    ...
    async def _analyze_task_staleness(self, task, session) -> Optional[Dict[str, Any]]:
        ...
        inactivity_threshold = timedelta(
            hours=self.stale_detection_config["inactivity_threshold_hours"]
        )
            ...
            stale_reasons.append(
                {
                    ...
                    "threshold_hours": self.stale_detection_config[
    ...
    async def _check_dependency_failures(self, task, session) -> List[Dict[str, Any]]:
        ...
        try:
                        ...
                        dependency_failures.append(
                            {
                                ...
                                "hours_waiting": time_since_activity.total_seconds()
                                / 3600,
                            }
                        )
            ...
            logger.warning(
                f"Error checking dependencies for task {task.task_id}: {str(e)}"
            )
    ...
    async def _is_orphaned_task(self, task, session) -> bool:
        ...
        try:
            ...
            logger.warning(
                f"Error checking orphan status for task {task.task_id}: {str(e)}"
            )
            return False

    def _determine_recommended_action(
        self, task, stale_reasons: List[Dict], severity: str
    ) -> str:
        ...
        if severity == "medium":
            ...
            for reason in stale_reasons:
                if (
                    reason.get("reason") == StaleTaskReason.INACTIVITY_TIMEOUT.value
                    and reason.get("duration_hours", 0) > 168
                ):  # 1 week
