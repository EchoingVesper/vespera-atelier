...
class WorkspaceCleanupService:
    ...
    async def perform_workspace_cleanup(
        ...
        try:
            ...
            if self.cleanup_config["database_optimization_enabled"]:
                ...
                result.database_optimizations = optimization_result[
                    "optimizations_applied"
                ]
                result.total_space_freed_mb = optimization_result.get(
                    "space_freed_mb", 0.0
                )
            ...
            logger.info(
                f"Workspace cleanup completed in {result.execution_time_seconds:.2f} seconds"
            )
            logger.info(
                f"Cleanup summary: {result.orphaned_tasks_removed} orphans removed, "
            ...
            async with self.db_manager.session_scope() as session:
                ...
                orphaned_query = select(SubTaskModel).where(
                    or_(
                        ...
                        ~select(TaskBreakdownModel.parent_task_id)
                        .where(
                            TaskBreakdownModel.parent_task_id
                            == SubTaskModel.parent_task_id
                        )
                        .exists(),
                    )
                )
                ...
                if not aggressive:
                    ...
                    orphaned_query = orphaned_query.where(
    ...
    async def _remove_orphaned_task_completely(self, task, session):
        ...
        try:
            ...
            if (
                hasattr(self.archival_service, "artifact_manager")
                and self.archival_service.artifact_manager
            ):
                try:
                    await self.archival_service.artifact_manager.delete_task_artifacts(
                        task_id
                    )
                except Exception as e:
                    logger.warning(
                        f"Failed to delete artifacts for task {task_id}: {str(e)}"
                    )
    ...
    async def _cleanup_task_associations(self, task, session):
        ...
        try:
            ...
            try:
                ...
                await session.execute(
                    delete(TaskAttributeModel).where(
                        TaskAttributeModel.task_id == task_id
                    )
                )
            except Exception:
                # Table might not exist in all configurations
            ...
            logger.warning(
                f"Some task associations could not be cleaned up for {task_id}: {str(e)}"
            )

    async def _resolve_incomplete_workflows(
        self, aggressive: bool = False
    ) -> Dict[str, Any]:
    ...
    async def _identify_incomplete_workflows(
        self, aggressive: bool = False
    ) -> List[Dict[str, Any]]:
            ...
            async with self.db_manager.session_scope() as session:
                ...
                for (
                    ...
                    if total_subtasks == 0:
                        ...
                        failed_subtasks > 0
                        and completed_subtasks + failed_subtasks == total_subtasks
                    ):
                        is_incomplete = True
    ...
    async def _resolve_incomplete_workflow(
        self, workflow_info: Dict[str, Any], aggressive: bool = False
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if reason == "no_subtasks" or (
                aggressive and workflow_info["completion_rate"] < 0.1
            ):
                ...
                success_rate = (
                    workflow_info["completed_subtasks"]
                    / workflow_info["total_subtasks"]
                )
                if success_rate >= 0.7:
        ...
        async with self.db_manager.session_scope() as session:
            ...
            result = await session.execute(
                select(SubTaskModel).where(
                    SubTaskModel.parent_task_id == parent_task_id
                )
            )
            ...
            for subtask in subtasks:
                if subtask.status not in ["archived", "completed"]:
                    ...
                    if not archive_result.success:
                        logger.warning(
                            f"Failed to archive subtask {subtask.task_id}: {archive_result.error}"
                        )
            ...
            await session.commit()
            logger.info(
                f"Archived entire workflow {parent_task_id} with {len(subtasks)} subtasks"
            )
        ...
        try:
            async with self.db_manager.session_scope() as session:
                ...
                if deleted_tracking.rowcount > 0:
                    ...
                    logger.info(
                        f"Cleaned up {deleted_tracking.rowcount} old tracking records"
                    )
            ...
            async with self.db_manager.session_scope() as session:
                ...
                for stale_task in stale_tasks:
                    ...
                    if tracking_record:
                        ...
                        tracking_record.auto_cleanup_eligible = stale_task[
                        ...
                        new_tracking = StaleTaskTrackingModel(
                            ...
                            detection_reason=(
                                stale_task["stale_reasons"][0]["reason"]
                                if stale_task["stale_reasons"]
                                else "unknown"
                            ),
