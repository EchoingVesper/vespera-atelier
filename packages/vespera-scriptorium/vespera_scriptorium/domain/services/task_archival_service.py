...
class TaskArchivalService:
    ...
    async def archive_task(
        self, task_id: str, reason: str = "stale", preserve_artifacts: bool = True
    ) -> ArchivalResult:
        """
        ...
        try:
            async with self.db_manager.session_scope() as session:
                ...
                logger.info(
                    f"Successfully archived task {task_id} with archive ID {archive_id}"
                )
            ...
            return ArchivalResult(success=False, task_id=task_id, error=str(e))
    ...
    async def _prepare_archive_data(self, task, session) -> Dict[str, Any]:
        ...
        archive_data = {
            ...
            "last_activity_at": (
                task.last_activity_at.isoformat() if task.last_activity_at else None
            ),
            "completed_at": (
        ...
        if task.parent_task_id:
            try:
                ...
                logger.warning(
                    f"Could not retrieve parent context for task {task.task_id}: {str(e)}"
                )
        ...
        if task.parent_task_id:
            try:
                ...
                archive_data["workflow_context"] = {
                    ...
                    "failed_siblings": len(
                        [t for t in sibling_tasks if t.status == "failed"]
                    ),
                ...
                logger.warning(
                    f"Could not retrieve workflow context for task {task.task_id}: {str(e)}"
                )

        return archive_data

    async def _create_archive_record(
        self, task, archive_data: Dict[str, Any], reason: str, session
    ) -> str:
    ...
    async def _create_archive_artifact(
        self, task, archive_data: Dict[str, Any], archive_id: str
    ) -> List[str]:
        ...
        try:
            ...
            archive_content = {
                ...
                "metadata": {
                    ...
                    "compression": self.archive_retention_config.get(
            ...
            logger.warning(
                f"Failed to create archive artifacts for task {task.task_id}: {str(e)}"
            )
                ...
                for archive in expired_archives:
                    try:
                        ...
                        if await self._should_preserve_archive(archive):
                            logger.info(
                                f"Preserving archive {archive.archive_id} due to preservation rules"
                            )
                            continue

                        # Delete associated artifacts
                        if self.artifact_manager and hasattr(
                            archive, "original_task_id"
                        ):
                            try:
                                deleted_artifacts = (
                                    await self.artifact_manager.delete_task_artifacts(
                                        archive.original_task_id
                                    )
                                )
                                cleanup_stats["artifacts_deleted"] += len(
                                    deleted_artifacts
                                )
                            except Exception as e:
                                cleanup_stats["errors"].append(
                                    f"Failed to delete artifacts for archive {archive.archive_id}: {str(e)}"
                                )
                        ...
                        cleanup_stats["errors"].append(
                            f"Failed to process archive {archive.archive_id}: {str(e)}"
                        )
    ...
    async def _should_preserve_archive(self, archive) -> bool:
        ...
        if (
            self.archive_retention_config.get("preserve_critical_artifacts", False)
            and archive.archive_size_bytes < 1024 * 1024
        ):  # < 1MB
            return True
    ...
    def _calculate_task_duration(self, archive_data: Dict[str, Any]) -> Optional[str]:
        ...
        try:
            ...
            activity_str = archive_data.get("last_activity_at") or archive_data.get(
                "completed_at"
            )
