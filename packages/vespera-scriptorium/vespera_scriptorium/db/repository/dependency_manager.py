...
async def add_dependency(repo_instance, dependency: TaskDependency) -> TaskDependency:
    ...
    async with repo_instance.get_session() as session:
        ...
        if await would_create_cycle(
            session, dependency.dependent_task_id, dependency.prerequisite_task_id
        ):
        ...
        # Save dependency
        from .helpers import record_event, save_dependency
...
async def check_dependencies(
    ...
    async with repo_instance.get_session() as session:
        ...
        for row in result:
            ...
            if dep.auto_satisfy and dep.can_satisfy(
                TaskStatus(row["prerequisite_status"])
            ):
                # Auto-satisfy the dependency
                await update_dependency_status(
                    session, dep.dependency_id, DependencyStatus.SATISFIED
                )
            else:
...
async def get_dependency_graph(
    repo_instance, root_task_id: Optional[str] = None
) -> Dict[str, List[str]]:
...
async def would_create_cycle(
    session: AsyncSession, dependent_id: str, prerequisite_id: str
) -> bool:
    ...
    result = await session.execute(
        stmt, {"start_id": prerequisite_id, "end_id": dependent_id}
    )
...
async def update_dependency_status(
    session: AsyncSession, dependency_id: int, status: DependencyStatus
):
