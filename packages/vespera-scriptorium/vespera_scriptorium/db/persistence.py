...
from ..domain.validation import ValidationError, validate_task_id
...
class DatabasePersistenceManager:
    ...
    def __init__(self, base_dir: Optional[str] = None, db_url: Optional[str] = None):
        ...
        self.db_url = (
            db_url or f"sqlite:///{self.base_dir}/.vespera_scriptorium/tasks.db"
        )
        self._repository = GenericTaskRepository(db_url=self.db_url)
        logger.info(
            f"DatabasePersistenceManager initialized with base_dir: {self.base_dir}"
        )
    ...
    async def cleanup_stale_locks(self, max_age_seconds: int = 120) -> int:
        ...
        try:
            ...
            if hasattr(self._repository, "cleanup_stale_locks"):
                cleaned_count = await self._repository.cleanup_stale_locks(
                    max_age_seconds
                )
                ...
                logger.warning(
                    "Repository does not support stale lock cleanup, using fallback"
                )
        ...
        try:
            ...
            task_data = {
                ...
                "status": (
                    ...
                    if hasattr(breakdown.status, "value")
                    else str(breakdown.status)
                ),
                "task_type": (
                    breakdown.task_type.value
                    if hasattr(breakdown.task_type, "value")
                    else str(breakdown.task_type)
                ),
                "specialist_type": (
                    breakdown.specialist_type.value
                    if hasattr(breakdown.specialist_type, "value")
                    else str(breakdown.specialist_type)
                ),
                "complexity": (
                    breakdown.complexity.value
                    if hasattr(breakdown.complexity, "value")
                    else str(breakdown.complexity)
                ),
                ...
                "due_date": (
                    breakdown.due_date.isoformat() if breakdown.due_date else None
                ),
                "created_at": (
                    breakdown.created_at.isoformat() if breakdown.created_at else None
                ),
                "updated_at": (
