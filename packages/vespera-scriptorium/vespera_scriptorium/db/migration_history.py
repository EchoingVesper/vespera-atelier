...
class MigrationHistoryManager:
    ...
    def record_migration_start(
        self, migration_record: MigrationRecord, batch_id: Optional[str] = None
    ) -> int:
        """
        ...
        try:
            ...
            logger.info(
                f"Recorded migration start: {migration_record.name} (ID: {migration_id})"
            )
            return migration_id
    ...
    def record_migration_success(self, migration_id: int, execution_time_ms: int):
        ...
        try:
            ...
            logger.info(
                f"Recorded migration success: ID {migration_id}, {execution_time_ms}ms"
            )
    ...
    def record_migration_failure(
        self, migration_id: int, error_message: str, execution_time_ms: int
    ):
        """
        ...
        try:
            ...
            logger.error(
                f"Recorded migration failure: ID {migration_id}, error: {error_message}"
            )
    ...
    def record_migration_rollback(self, migration_id: int, rollback_reason: str):
        ...
        try:
            update_stmt = (
                update(self.history_table)
                .where(self.history_table.c.id == migration_id)
                .values(status="rolled_back", error_message=rollback_reason)
            )
            ...
            logger.info(
                f"Recorded migration rollback: ID {migration_id}, reason: {rollback_reason}"
            )
    ...
    def get_migration_history(
        ...
        """
        try:
            select_stmt = select(self.history_table).order_by(
                self.history_table.c.applied_at.desc()
            )

            if status_filter:
                select_stmt = select_stmt.where(
                    self.history_table.c.status == status_filter
                )
    ...
    def get_failed_migrations(self, since_hours: int = 24) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(
                f"Retrieved {len(records)} failed migrations in last {since_hours} hours"
            )
            return records
    ...
    def _generate_batch_id(self) -> str:
        ...
        random_part = hashlib.md5(str(datetime.now().microsecond).encode()).hexdigest()[
            :8
        ]
