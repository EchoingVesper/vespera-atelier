...
class AutoMigrationSystem:
    ...
    def check_migration_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            failed_migrations = self.history_manager.get_failed_migrations(
                since_hours=24
            )
            ...
            logger.info(
                f"Migration status check complete in {check_time_ms}ms: "
    ...
    def execute_auto_migration(self, force_backup: bool = None) -> MigrationResult:
        ...
        try:
            ...
            backup_enabled = (
                force_backup if force_backup is not None else self.auto_backup
            )
            if backup_enabled:
                batch_id = self.history_manager._generate_batch_id()
                backup_info = self.backup_manager.create_backup(
                    self.database_url, migration_batch_id=batch_id
                )
            ...
            operations = self.migration_manager.generate_migration_operations(
                differences
            )
            ...
            if self.dry_run_mode:
                logger.info(
                    f"DRY RUN: Would execute {len(operations)} migration operations"
                )
                result.success = True
                result.execution_time_ms = int((time.time() - start_time) * 1000)
                return result
            ...
            if migration_success:
                result.success = True
                logger.info(
                    f"Migration completed successfully: {len(operations)} operations"
                )
            else:
    ...
    def _execute_with_tracking(
        self, operations: List[MigrationOperation], result: MigrationResult
    ) -> bool:
        ...
        try:
            ...
            for i, operation in enumerate(operations):
                migration_record = MigrationRecord(
                    ...
                    checksum=self.history_manager.calculate_checksum(
                ...
                # Record start
                migration_id = self.history_manager.record_migration_start(
                    migration_record, batch_id
                )
                ...
                if success:
                    self.history_manager.record_migration_success(
                        migration_id, operation_time_ms
                    )
                    logger.info(
                        f"Operation {i+1}/{len(operations)} completed: {operation.operation_type}"
                    )
                else:
                    ...
                    self.history_manager.record_migration_failure(
                        migration_id, error_msg, operation_time_ms
                    )
    ...
    def rollback_last_migration(self, use_backup: bool = True) -> Dict[str, Any]:
        ...
        try:
            ...
            if not recent_migrations:
                return {"success": False, "error": "No migrations found to rollback"}

            last_migration = recent_migrations[0]
            migration_id = last_migration["id"]

            # Execute rollback
            rollback_result = self.rollback_manager.rollback_migration(
                migration_id, use_backup
            )
    ...
    def _calculate_health_score(
        self, migration_stats: Dict[str, Any], failed_migrations: List[Dict]
    ) -> int:
    ...
    def _generate_health_recommendations(
        ...
        if failed_migrations:
            recommendations.append(
                f"Review {len(failed_migrations)} recent migration failures"
            )

        success_rate = migration_stats.get("success_rate", 100)
        if success_rate < 90:
            recommendations.append(
                f"Migration success rate is low ({success_rate:.1f}%) - investigate causes"
            )

        avg_time = migration_stats.get("average_execution_time_ms", 0)
        if avg_time > 10000:
            recommendations.append(
                "Average migration time is high - consider optimization"
            )
...
def execute_startup_migration(
    database_url: str, backup_directory: Optional[Path] = None
) -> MigrationResult:
    """
    ...
    try:
        ...
        if result.success:
            if result.migration_needed:
                logger.info(
                    f"Startup migration completed: {result.operations_executed} operations in {result.execution_time_ms}ms"
                )
            else:
