...
// Helper async function (not a standalone test)
...
async fn run_all_memory_tests() {
    ...
    test_operation_log_bounded_growth();
    test_metadata_layer_garbage_collection();
    test_reference_layer_garbage_collection();
    test_text_layer_cleanup();
    test_codex_manager_memory_management().await;
    test_sync_manager_weak_references().await;
    test_connection_lifecycle().await;
    test_comprehensive_resource_cleanup();
